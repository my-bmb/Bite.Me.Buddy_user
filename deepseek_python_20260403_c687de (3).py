...
app.permanent_session_lifetime = timedelta(days=7)  # ✅ Now this works
...
def checkout():
    if request.method == 'POST':
        ...
        try:
            ...
            items_json = json.dumps(items_list)  # ✅ This ensures list format
...
def order_history():
    try:
        ...
        for index, order in enumerate(orders_data):
            ...
            # Parse items JSON with error handling
            items_list = []
            if order.get('items'):
                try:
                    json_items = json.loads(order['items'])
                    
                    # ✅ FIX: Handle dict by converting to list
                    ...
                    print(f"❌ [ORDER_HISTORY] JSON decode error for order {order['order_id']}: {e}")
                    items_list = []
                except Exception as e:
                    print(f"❌ [ORDER_HISTORY] Error parsing items: {e}")
                    items_list = []
...
# ============================================
# ✅ ORDER DETAILS ROUTE - SUPABASE (COMPLETELY FIXED)
# ============================================

@app.route('/order/<int:order_id>')
@login_required
def order_details(order_id):
    """View detailed order information with IST timezone conversion - FIXED for service orders"""
    try:
        ...
        # ✅ CRITICAL FIX: Parse items JSON - Handle BOTH dict and list formats
        items_list = []
        if order.get('items'):
            try:
                json_items = json.loads(order['items'])
                print(f"🔍 [ORDER_DETAILS] Items JSON loaded, type: {type(json_items)}")
                print(f"🔍 [ORDER_DETAILS] RAW ITEMS: {str(json_items)[:200]}...")
                
                # ✅ FIX: Handle dict (single item) by converting to list
                if isinstance(json_items, dict):
                    print(f"⚠️ [ORDER_DETAILS] Items is dict, converting to list")
                    json_items = [json_items]
                
                # Now process as list
                if isinstance(json_items, list):
                    for idx, item in enumerate(json_items):
                        print(f"🔍 [ORDER_DETAILS] Item {idx}: {item.get('item_name', item.get('name', 'Unknown'))}")
                        
                        # Create safe item dictionary with all required fields
                        safe_item = {
                            'name': item.get('item_name', item.get('name', 'Unknown Item')),
                            'item_name': item.get('item_name', item.get('name', 'Unknown Item')),
                            'type': item.get('item_type', item.get('type', 'unknown')),
                            ...
                            'photo': item.get('item_photo', item.get('photo', '')),
                            'item_photo': item.get('item_photo', item.get('photo', '')),
                            'description': item.get('item_description', item.get('description', 'No description')),
                            'item_description': item.get('item_description', item.get('description', 'No description')),
                            'quantity': int(item.get('quantity', 1)),
                            'price': float(item.get('price', 0)),
                            'total': float(item.get('total', item.get('price', 0) * item.get('quantity', 1)))
                        }
                        items_list.append(safe_item)
                else:
                    print(f"❌ [ORDER_DETAILS] Items is neither list nor dict: {type(json_items)}")
                    items_list = []
                    
            except json.JSONDecodeError as e:
                print(f"❌ [ORDER_DETAILS] JSON decode error for order {order_id}: {e}")
                print(f"   Items string: {str(order['items'])[:200]}...")
                items_list = []
            except Exception as e:
                print(f"❌ [ORDER_DETAILS] Error parsing items: {e}")
                traceback.print_exc()
                items_list = []
        
        print(f"✅ [ORDER_DETAILS] Parsed {len(items_list)} items")
...
# ============================================
# ✅ ORDERS REDIRECT ROUTE - FIXED (Added to fix url_for('orders') error)
# ============================================
