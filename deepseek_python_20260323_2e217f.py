# app.py - COMPLETE UPDATED VERSION WITH FULL COLUMN FETCH FOR SERVICES & MENU
import os
...
# ============================================
# ✅ SERVICES SYSTEM - UPDATED (Full column fetch)
# ============================================

@app.route('/services')
@login_required
def services():
    """Display all service categories with their photos (main services page)"""
    print("\n🔍 [SERVICES] Fetching service categories from Supabase...")
    try:
        # Fetch all active service categories from Supabase with ALL columns
        ...
        print(f"📊 [SERVICES] Found {len(categories) if categories else 0} service categories")
        
        # Sort by position
        if categories:
            categories = sorted(categories, key=lambda x: (x.get('position', 0), x.get('name', '')))
            print(f"📊 [SERVICES] Categories sorted: {[cat.get('name') for cat in categories]}")
            # Print all columns for first category
            if len(categories) > 0:
                print(f"📊 [SERVICES] Category columns: {list(categories[0].keys())}")
        else:
            categories = []
        
        # For each category, get service count and full service details
        for category in categories:
            # Get ALL services in this category with ALL columns
            services_in_category = supabase_execute(
                'services',
                'select',
                conditions={'category_id': category['id'], 'status': 'active'}
            )
            category['service_count'] = len(services_in_category) if services_in_category else 0
            # Store full services data for potential use
            category['services_data'] = services_in_category if services_in_category else []
            print(f"  - Category: {category.get('name')} (ID: {category['id']}) - {category['service_count']} services")
            
            # If no category photo, use default
        ...
        print(f"✅ [SERVICES] Returning {len(categories)} categories to template")
        return render_template('services.html', categories=categories)
        
    except Exception as e:
        print(f"❌ [SERVICES] Error loading service categories: {e}")
        traceback.print_exc()
        flash('Error loading service categories', 'error')
        return render_template('services.html', categories=[])

@app.route('/service-category/<int:category_id>')
@login_required
def service_category_items(category_id):
    """Display all services for a specific category with ALL columns"""
    print(f"\n🔍 [SERVICE-CATEGORY] Fetching services for category ID: {category_id}")
    try:
        ...
        if not categories:
            print(f"❌ [SERVICE-CATEGORY] Category {category_id} not found")
            flash('Service category not found', 'error')
            return redirect(url_for('services'))
        
        category = categories[0]
        print(f"📊 [SERVICE-CATEGORY] Category: {category.get('name')} (ID: {category['id']})")
        print(f"📊 [SERVICE-CATEGORY] Category columns: {list(category.keys())}")
        
        # Get ALL services for this category with ALL columns
        ...
        print(f"📊 [SERVICE-CATEGORY] Found {len(services_list) if services_list else 0} services in this category")
        
        # Sort by position
        if services_list:
            services_list = sorted(services_list, key=lambda x: (x.get('position', 0), x.get('name', '')))
            for idx, service in enumerate(services_list):
                print(f"  Service {idx+1}: {service.get('name')} - Price: {service.get('price', 0)} - Discount: {service.get('discount', 0)}% - Final: {service.get('final_price', 0)}")
                print(f"    Service columns: {list(service.keys())}")
        
        # If no photo in DB, try Cloudinary
        for service in services_list:
            if not service.get('photo') or service['photo'] == '':
                try:
                    ...
                    print(f"⚠️ Cloudinary error for {service['name']}: {e}")
                    service['photo'] = "https://res.cloudinary.com/demo/image/upload/v1633427556/sample_service.jpg"
        
        # Get all categories for horizontal scroll (bottom section)
        all_categories = supabase_execute(
            'service_categories',
            'select',
            conditions={'status': 'active'}
        )
        print(f"📊 [SERVICE-CATEGORY] Found {len(all_categories) if all_categories else 0} total categories for horizontal scroll")
        ...
        print(f"✅ [SERVICE-CATEGORY] Returning {len(services_list)} services for category {category.get('name')}")
        return render_template('service_category_items.html', 
                             category=category, 
                             services=services_list,
                             all_categories=all_categories)
        
    except Exception as e:
        print(f"❌ [SERVICE-CATEGORY] Error loading category services: {e}")
        traceback.print_exc()
...
# ============================================
# ✅ MENU SYSTEM - UPDATED (Full column fetch)
# ============================================

@app.route('/menu')
@login_required
def menu():
    """Display all menu categories with their photos (main menu page)"""
    print("\n🔍 [MENU] Fetching menu categories from Supabase...")
    try:
        # Fetch all active categories from Supabase with ALL columns
        categories = supabase_execute(
            'menu_categories',
            'select',
            conditions={'status': 'active'},
        ...
        print(f"📊 [MENU] Found {len(categories) if categories else 0} menu categories")
        
        # Sort by position
        if categories:
            categories = sorted(categories, key=lambda x: (x.get('position', 0), x.get('name', '')))
            print(f"📊 [MENU] Categories sorted: {[cat.get('name') for cat in categories]}")
            # Print all columns for first category
            if len(categories) > 0:
                print(f"📊 [MENU] Category columns: {list(categories[0].keys())}")
        else:
            categories = []
        
        # For each category, get item count and full menu items details
        for category in categories:
            # Get ALL menu items in this category with ALL columns
            items_in_category = supabase_execute(
                'menu_items',
                'select',
                conditions={'category_id': category['id'], 'status': 'active'}
            )
            category['item_count'] = len(items_in_category) if items_in_category else 0
            # Store full menu items data for potential use
            category['menu_items_data'] = items_in_category if items_in_category else []
            print(f"  - Category: {category.get('name')} (ID: {category['id']}) - {category['item_count']} items")
            
            # If no category photo, use default
            if not category.get('category_photo'):
                category['category_photo'] = "https://res.cloudinary.com/demo/image/upload/v1633427556/default_category.jpg"
        
        print(f"✅ [MENU] Returning {len(categories)} categories to template")
        return render_template('menu.html', categories=categories)
        
    except Exception as e:
        print(f"❌ [MENU] Error loading menu categories: {e}")
        traceback.print_exc()
        flash('Error loading menu categories', 'error')
        return render_template('menu.html', categories=[])

@app.route('/menu-category/<int:category_id>')
@login_required
def menu_category_items(category_id):
    """Display all menu items for a specific category with ALL columns"""
    print(f"\n🔍 [MENU-CATEGORY] Fetching menu items for category ID: {category_id}")
    try:
        # Get category details with ALL columns
        categories = supabase_execute(
            'menu_categories',
        ...
        if not categories:
            print(f"❌ [MENU-CATEGORY] Category {category_id} not found")
            flash('Category not found', 'error')
            return redirect(url_for('menu'))
        
        category = categories[0]
        print(f"📊 [MENU-CATEGORY] Category: {category.get('name')} (ID: {category['id']})")
        print(f"📊 [MENU-CATEGORY] Category columns: {list(category.keys())}")
        
        # Get ALL menu items for this category with ALL columns
        ...
        print(f"📊 [MENU-CATEGORY] Found {len(menu_items) if menu_items else 0} menu items in this category")
        
        # Sort by position
        if menu_items:
            menu_items = sorted(menu_items, key=lambda x: (x.get('position', 0), x.get('name', '')))
            for idx, item in enumerate(menu_items):
                print(f"  Item {idx+1}: {item.get('name')} - Price: {item.get('price', 0)} - Discount: {item.get('discount', 0)}% - Final: {item.get('final_price', 0)}")
                print(f"    Menu item columns: {list(item.keys())}")
        
        # If no photo in DB, try Cloudinary
        for item in menu_items:
            if not item.get('photo') or item['photo'] == '':
                try:
                    search_name = item['name'].lower().replace(' ', '_')
                    search_result = cloudinary.Search()\
                        .expression(f"folder:menu_items AND filename:{search_name}*")\
                        .execute()
                    
                    if search_result['resources']:
                        item['photo'] = search_result['resources'][0]['secure_url']
                        print(f"  📸 Loaded Cloudinary photo for {item['name']}")
                    else:
                        item['photo'] = "https://res.cloudinary.com/demo/image/upload/v1633427556/sample_food.jpg"
                except Exception as e:
                    print(f"⚠️ Cloudinary error for {item['name']}: {e}")
                    item['photo'] = "https://res.cloudinary.com/demo/image/upload/v1633427556/sample_food.jpg"
        
        # Get all categories for horizontal scroll (bottom section) with ALL columns
        all_categories = supabase_execute(
            'menu_categories',
            'select',
            conditions={'status': 'active'}
        )
        print(f"📊 [MENU-CATEGORY] Found {len(all_categories) if all_categories else 0} total categories for horizontal scroll")
        ...
        print(f"✅ [MENU-CATEGORY] Returning {len(menu_items)} items for category {category.get('name')}")
        ...
        print(f"❌ [MENU-CATEGORY] Error loading category items: {e}")
        traceback.print_exc()
        flash('Error loading menu items', 'error')
        return redirect(url_for('menu'))
...
def debug_data():
    ...
    try:
        ...
        debug_info['service_categories'] = {
            ...
            'data': categories[:5] if categories else []  # Show first 5
        }
        ...
        column_info = {}
        tables = ['service_categories', 'services', 'menu_categories', 'menu_items']
