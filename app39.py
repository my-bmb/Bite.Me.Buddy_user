# app.py - COMPLETELY FIXED WITH ORDER HISTORY WORKING
import os
...
# ============================================
# ✅ FIXED CHECKOUT ROUTE WITH PROPER JSON SAVING
# ============================================

@app.route('/checkout', methods=['GET', 'POST'])
@login_required
def checkout():
    """Order checkout and confirmation - FIXED JSON SAVING"""
    if request.method == 'POST':
        ...
        try:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    print(f"🔍 [CHECKOUT] Found {len(cart_items)} cart items for user {session['user_id']}")
                    ...
                    print(f"🔍 [CHECKOUT] Creating order with items JSON:")
                    print(items_json[:500] + "..." if len(items_json) > 500 else items_json)
                    ...
                    print(f"✅ [CHECKOUT] Added {len(cart_items)} items to order_items table")
...
@login_required
def order_history():
    """Display user's order history - COMPLETELY FIXED"""
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                # ✅ GET ORDERS WITH FORMATTED DATE
                cur.execute("""
                    SELECT o.order_id, o.total_amount, o.payment_mode, 
                ...
                for order in orders:
                    # ✅ ALWAYS GET ITEMS FROM ORDER_ITEMS TABLE (GUARANTEED WORKING)
                    cur.execute("""
                        SELECT oi.item_type, oi.quantity, oi.price,
                               COALESCE(s.name, m.name) as item_name
                        FROM order_items oi
                    ...
                    print(f"🔍 [ORDER_HISTORY] Order #{order['order_id']} has {len(items_list)} items")
                    
                    # Format date
                    order_date = order.get('formatted_date')
                    if not order_date and order.get('order_date'):
                        try:
                    ...
                    orders_list.append({
                        ...
                        'items': items_list  # ✅ ALWAYS FROM ORDER_ITEMS TABLE
                    })
...
# ============================================
# ✅ DEBUG ROUTE FOR CHECKING ORDERS
# ============================================
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    SELECT o.order_id, o.items, o.total_amount,
                           o.status, o.order_date,
                           LENGTH(o.items) as items_length,
                           LEFT(o.items, 200) as items_preview
                    FROM orders o
                    WHERE o.user_id = %s
                    ORDER BY o.order_date DESC
                    LIMIT 3
                ...
                for order in sample_orders:
                    ...
                    order_items_data[order['order_id']] = cur.fetchall()
