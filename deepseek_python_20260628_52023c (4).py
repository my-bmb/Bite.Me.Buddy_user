# app.py - COMPLETE HYBRID AUTH VERSION (Session for Ecommerce + Flask-Login for Chat)
# ✅ Ecommerce routes use session-based authentication (session['user_id'])
# ✅ Chat routes use Flask-Login (current_user)
# ✅ Both systems share the same users table
# ✅ All existing features preserved
# ✅ No code was deleted – only additions and selective replacement of decorators
# ✅ Fixed logout – properly deletes remember_token and session cookies with correct attributes
...
# ✅ FIXED: users_chat route now uses Flask-Login current_user.id
# ✅ FIXED: SocketIO connect/disconnect now use Flask-Login current_user
# ✅ FIXED: get_unread_counts now handles integer IDs gracefully
# ✅ FIXED: view_user_profile now uses Flask-Login current_user
# ✅ FIXED: update_location now uses Flask-Login current_user
# ✅ FIXED: live_chat now uses Flask-Login current_user and constructs user object
# ✅ FIXED: url_for('users') -> url_for('users_chat') in all chat-related redirects
# ✅ FIXED: global live_chat_cache in get_live_messages()
# ✅ FINAL FIX: chat route now uses Flask-Login current_user
# ✅ ADDED: /api/cart/count endpoint
# ✅ FIXED: profile_chat, group_video_call, audio_call, video_call, edit_profile now Flask-Login
# ✅ FIXED: All socketio events now use Flask-Login current_user
...
# ✅ FIXED: Logout issue due to persistent remember me cookie - now deletes cookie with correct attributes
# ✅ HYBRID: Ecommerce routes use @session_required, Chat routes use @login_required

# ============================================================
# 🔐 HYBRID AUTH SYSTEM
# - Ecommerce: session-based (session['user_id'])
# - Chat: Flask-Login (current_user)
# - Both share the same users table
# ============================================================
...
# ✅ Flask-Login Setup (used only for chat routes)
login_manager = LoginManager(app)
login_manager.login_view = 'login'  # Shared login page
login_manager.login_message = 'Please login to access this page'
login_manager.login_message_category = 'warning'

# ============================================================
# ✅ NEW: BEFORE_REQUEST TO ENSURE SESSION FROM FLASK-LOGIN
# ============================================================
@app.before_request
def ensure_session_from_login():
    """
    If a user is authenticated via Flask-Login but session is missing,
    populate the session from the user's database record.
    This prevents redirect loops when accessing ecommerce routes after chat login.
    """
    # Skip these endpoints to avoid interfering with login/register flows
    if request.endpoint in ['login', 'register', 'static', 'health', 'ping', 'debug_session']:
        return
    ...
    if current_user.is_authenticated and 'user_id' not in session:
        try:
            ...
            if not user_id:
                # If user has no id, log them out
                logout_user()
                session.clear()
                return
            ...
            if result.data:
                ...
                session['wallet_balance'] = user.get('wallet_balance', 0)
                session['referral_count'] = user.get('referral_count', 0)
                session['total_spent'] = user.get('total_spent', 0)
                session['profile_pic'] = user.get('profile_pic')
                session['location'] = user.get('location')
                session['latitude'] = user.get('latitude')
                session['longitude'] = user.get('longitude')
                if user.get('created_at'):
                    session['created_at'] = format_ist_datetime(user['created_at'], "%d %b %Y")
                session.permanent = True
                print(f"✅ Session populated for user {user_id} from Flask-Login")
            else:
                # User data not found – log out
                print(f"⚠️ User {user_id} not found in DB, logging out")
                logout_user()
                session.clear()
        except Exception as e:
            print(f"❌ Session populate error: {e}")
            # Log out to break the loop
            logout_user()
            session.clear()

# ============================================================
# ✅ FIXED SESSION_REQUIRED DECORATOR (now also populates session if needed)
# ============================================================
def session_required(f):
    """Decorator for ecommerce routes that require session-based authentication.
    Will also populate session from Flask-Login if possible."""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if 'user_id' not in session:
            # If Flask-Login has a user, try to populate session from it
            if current_user.is_authenticated:
                try:
                    ...
                    print(f"❌ Decorator session populate error: {e}")
                    logout_user()
                    session.clear()
...
# All user identity now from current_user (chat) or session['user_id'] (ecommerce).

# ============================================
# ✅ SESSION-BASED AUTH DECORATOR FOR ECOMMERCE ROUTES
# ============================================

# Decorator already defined above (session_required) – now updated.
...
def register():
    ...
    if request.method == 'POST':
        ...
        # ✅ MODIFIED: Use Supabase Storage for profile picture (instead of Cloudinary)
        ...
        if 'profile_pic' in request.files:
            ...
            if file and file.filename and allowed_file(file.filename):
                try:
                    ...
                    file_path = f"profile_photos/{phone}/{filename}"
                    file_bytes = file.read()
                    supabase.storage.from_('chat-files').upload(file_path, file_bytes)
                    profile_pic = supabase.storage.from_('chat-files').get_public_url(file_path)
                except Exception as e:
        ...
        try:
            ...
            if new_user and len(new_user) > 0:
                ...
                # ✅ Set both session and Flask-Login
                # Session for ecommerce
                ...
                print(f"  - Location string: {location}")
                print(f"  - Latitude saved: {user_data.get('latitude', 'NOT SAVED')}")
                print(f"  - Longitude saved: {user_data.get('longitude', 'NOT SAVED')}")
                print(f"  - Referral mobile: {referral_mobile if referral_mobile else 'None'}")
...
def login():
    # First, ensure session is populated if user is authenticated via Flask-Login
    if current_user.is_authenticated and 'user_id' not in session:
        try:
            user_id = current_user.id
            result = supabase.table('users').select('*').eq('id', user_id).execute()
            if result.data:
                ...
                if user.get('created_at'):
                    session['created_at'] = format_ist_datetime(user['created_at'], "%d %b %Y")
                session.permanent = True
                print(f"Login: Session populated for user {user_id}")
        except Exception as e:
            print(f"Login session populate error: {e}")
            logout_user()
            session.clear()
        ...
        try:
            ...
            if users and len(users) > 0:
                ...
                if check_password_hash(user['password'], password):
                    ...
                    # Emit user status via socketio (chat)
...
# ============================================
# ✅ FIXED LOGOUT ROUTE - clears both session and Flask-Login with proper cookie deletion
# ============================================
...
def logout():
    ...
    # Clear session (removes all session data including ecommerce user_id)
    ...
    # Delete remember_token cookie with the same attributes it was set with
    print("🍪 Deleting remember_token cookie...")
    response.delete_cookie(
        'remember_token',
        path='/',
        domain=None,  # Use None to match the current domain
        secure=app.config['SESSION_COOKIE_SECURE'],  # Matches Flask-Login's setting
        httponly=True,
        samesite='Lax'
    )
    print("✅ remember_token cookie deleted.")

    # Also delete the session cookie explicitly
...
# ============================================
# ✅ ECOMMERCE ROUTES - SESSION-BASED AUTH
# ============================================
...
# Service Collection Categories (session)
...
# Service Category Services (session)
...
# Goods Collection Categories (session)
...
def checkout():
        ...
        try:
            ...
            if payment_mode == 'online':
                ...
                if reward_result:
                    ...
                    session['wallet_balance'] = session.get('wallet_balance', 0) + reward_result['amount']
    ...
    try:
        ...
        # Get user's saved coordinates from session
...
def profile():
    if request.method == 'POST':
        ...
        try:
            ...
            # Also update Flask-Login user object if needed (for chat)
            if current_user.is_authenticated and current_user.id == session['user_id']:
                ...
                current_user.location = parsed_location['address']
                if parsed_location['is_auto_detected']:
                    current_user.latitude = parsed_location['latitude']
                    current_user.longitude = parsed_location['longitude']
...
def request_withdrawal():
    ...
    try:
        ...
        if amount > session.get('wallet_balance', 0):
        ...
        if result['success']:
            ...
            session['wallet_balance'] = session.get('wallet_balance', 0) - amount
...
def get_referral_stats():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'referral_count': session.get('referral_count', 0),
            'wallet_balance': session.get('wallet_balance', 0),
...
def verify_razorpay_payment():
    ...
    try:
        ...
        reward_result = process_referral_reward(session['user_id'], amount)
...
# Cloudinary Profile Picture Upload (session)
...
# Service & Goods Details API (session)
...
# ============================================================
# ✅ NEW: DEBUG SESSION ROUTE (to inspect authentication state)
# ============================================================
@app.route('/debug-session')
def debug_session():
    """Check session and current_user status for debugging redirect loops"""
    import json
    session_data = dict(session)
    # Hide sensitive data
    if 'user_id' in session_data:
        session_data['user_id'] = session_data['user_id']
    auth_status = {
        'is_authenticated': current_user.is_authenticated,
        'user_id': current_user.id if current_user.is_authenticated else None,
        'username': current_user.full_name if current_user.is_authenticated else None,
    }
...
def debug_create_test_order():
    ...
    try:
        ...
        order_data = {
            'user_id': user_id,
            'user_name': session.get('full_name', 'Test User'),
            'user_email': session.get('email', 'test@example.com'),
            'user_phone': session.get('phone', '9999999999'),
...
def debug_rls_status():
    ...
    try:
        ...
        user_orders = supabase_execute('orders', 'select', conditions={'user_id': current_user_id}, use_admin=False)
...
# ============================================
# ✅ ULTRA-FAST ROUTES (session-based)
# ============================================
...
# ============================================
# ✅ FIXED: users_chat route - using Flask-Login current_user
# ============================================
...
# ============================================
# ✅ FIXED: chat route - now uses Flask-Login current_user
# ============================================
...
# ============================================
# ✅ PRIVATE CHAT ROUTES (Flask-Login)
# ============================================
...
# ============================================
# ✅ EDIT PROFILE (CHAT) - Flask-Login based
# ============================================
...
def edit_profile():
    ...
    if request.method == 'POST':
        try:
            ...
            delete_profile_pic = request.form.get('delete_profile_pic', '0') == '1'
            ...
            # 1. Check if user wants to delete the profile picture
            if delete_profile_pic:
                profile_pic_url = DEFAULT_AVATAR_URL
            else:
            ...
            # 3. If no new file and not deleting, keep existing (fetch from DB)
            if profile_pic_url is None:
                existing_user = supabase.table('users').select('profile_pic').eq('id', user_id).execute()
            ...
            # Keep existing gallery photos if new ones are uploaded, otherwise keep old ones
            if existing_photos:
                photos_json = json.dumps(existing_photos)
            else:
                existing_user = supabase.table('users').select('photos').eq('id', user_id).execute()
                if existing_user.data:
            ...
            # Also update session for ecommerce consistency
            session['full_name'] = full_name
            if profile_pic_url:
...
# ============================================
# ✅ FIXED: view_user_profile - using Flask-Login current_user
# ============================================
...
# ============================================
# ✅ PROFILE CHAT ROUTE - Flask-Login based
# ============================================
...
# ============================================
# ✅ LIVE CHAT ROUTES (Flask-Login based)
# ============================================
...
# ============================================
# ✅ GROUP VIDEO CALL ROUTES (Flask-Login based)
# ============================================
...
# ============================================
# ✅ SOCKETIO EVENTS - ALL Flask-Login BASED
# ============================================
...
# ============================================
# ✅ GALLERY PHOTO DELETE ROUTE (NEW)
# ============================================

@app.route('/delete_gallery_photo', methods=['POST'])
@login_required
def delete_gallery_photo():
    """Delete a single gallery photo from user's photos list"""
    user_id = current_user.id
    data = request.get_json()
    photo_url = data.get('photo_url')
    
    if not photo_url:
        return jsonify({'success': False, 'error': 'No photo URL provided'}), 400
    
    try:
        # Get current photos list
        result = supabase.table('users').select('photos').eq('id', user_id).execute()
        if not result.data:
            return jsonify({'success': False, 'error': 'User not found'}), 404
        
        current_photos_json = result.data[0].get('photos')
        photos = json.loads(current_photos_json) if current_photos_json else []
        
        # Remove the photo URL from the list
        if photo_url not in photos:
            return jsonify({'success': False, 'error': 'Photo not found in gallery'}), 404
        
        photos.remove(photo_url)
        
        # Optional: Delete file from Supabase storage to save space
        # Extract file path from URL (e.g., https://.../profile_photos/123/photo.jpg)
        # Note: This is optional – you can keep the file and just remove from list
        try:
            # Extract path: everything after the bucket name
            # Assuming URL format: https://<project>.supabase.co/storage/v1/object/public/chat-files/profile_photos/123/photo.jpg
            if 'chat-files' in photo_url:
                # Get the path after 'chat-files/'
                path_parts = photo_url.split('chat-files/')
                if len(path_parts) > 1:
                    file_path = path_parts[1]
                    supabase.storage.from_('chat-files').remove([file_path])
                    logger.info(f"Deleted file from storage: {file_path}")
        except Exception as storage_error:
            # If storage deletion fails, still remove from list (soft delete)
            logger.warning(f"Could not delete file from storage: {storage_error}")
        
        # Update DB with new list
        new_photos_json = json.dumps(photos)
        supabase.table('users').update({'photos': new_photos_json}).eq('id', user_id).execute()
        
        # Update current_user object if needed
        if hasattr(current_user, 'photos'):
            current_user.photos = photos
        
        return jsonify({'success': True, 'message': 'Photo deleted successfully', 'photos': photos})
...
if __name__ == '__main__':
    ...
    if not is_render:
        ...
        print(f"✅ FIXED: users_chat route now uses Flask-Login current_user.id")
        print(f"✅ FIXED: SocketIO connect/disconnect now use Flask-Login current_user")
        print(f"✅ FIXED: get_unread_counts handles integer IDs gracefully")
        print(f"✅ FIXED: view_user_profile uses Flask-Login current_user")
        print(f"✅ FIXED: update_location uses Flask-Login current_user")
        print(f"✅ FIXED: live_chat uses Flask-Login current_user and constructs user object")
        ...
        print(f"✅ FINAL FIX: chat route now uses Flask-Login current_user")
        print(f"✅ ADDED: /api/cart/count endpoint")
        print(f"✅ FIXED: profile_chat, group_video_call, audio_call, video_call, edit_profile now Flask-Login")
        print(f"✅ FIXED: All socketio events now use Flask-Login current_user")
        print(f"✅ FIXED: All HTTP routes (live chat, private chat, uploads, edit/delete/reactions) now use Flask-Login current_user")
        ...
        print("✅ HYBRID AUTH: Ecommerce routes use session-based auth, Chat routes use Flask-Login")
        print("✅ Ecommerce routes use @session_required decorator")
        print("✅ Chat routes use @login_required (Flask-Login)")
        print("✅ Both systems share the same users table")
        print("✅ Logout clears both session and Flask-Login user")
        # ✅ FIXED: Logout route now clears remember_token cookie
        print("✅ FIXED: Logout route - remember_token deleted, session cleared")
        print("✅ ADDED: Gallery photo delete route (/delete_gallery_photo)")
        ...
        print("✅ FIXED: users_chat route now uses Flask-Login current_user.id")
        print("✅ FIXED: SocketIO connect/disconnect now use Flask-Login current_user")
        print("✅ FIXED: get_unread_counts handles integer IDs gracefully")
        print("✅ FIXED: view_user_profile uses Flask-Login current_user")
        print("✅ FIXED: update_location uses Flask-Login current_user")
        print("✅ FIXED: live_chat uses Flask-Login current_user and constructs user object")
        ...
        print("✅ FINAL FIX: chat route now uses Flask-Login current_user")
        print("✅ ADDED: /api/cart/count endpoint")
        print("✅ FIXED: profile_chat, group_video_call, audio_call, video_call, edit_profile now Flask-Login")
        print("✅ FIXED: All socketio events now use Flask-Login current_user")
        print("✅ FIXED: All HTTP routes (live chat, private chat, uploads, edit/delete/reactions) now use Flask-Login current_user")
