# app.py - COMPLETE OPTIMIZED VERSION WITH ALL FIXES
# ✅ Fixed: url_for('users') replaced with url_for('users_chat') everywhere
# ✅ Fixed: Added global live_chat_cache in get_live_messages()
# ✅ Fixed: Added /users redirect route for backward compatibility
# ✅ Fixed: Added /api/cart/count endpoint
# ✅ Fixed: SocketIO connection uses session-based auth (already present)
# ✅ Fixed: all live-chat routes use session instead of current_user
...
# ✅ FIXED: update_location uses session-based user_id
...
def update_location():
    ...
    # ✅ Use session user_id instead of current_user
    user_id = session.get('user_id')
    if not user_id:
...
def edit_profile():
    if request.method == 'POST':
        try:
            ...
            existing_user = supabase.table('users').select('photos', 'profile_pic').eq('id', current_user.id).execute()
...
# ✅ FIXED: view_user_profile redirect uses users_chat instead of users
...
live_chat_cache = []   # <-- Ensure this is defined before any function uses it
...
# ✅ FIXED: get_live_messages with global declaration
def get_live_messages(limit=20, offset=0, from_cache=False):
    """Get live chat messages from database"""
    global live_chat_cache  # ✅ Added global declaration
...
# ============================================
# ✅ LIVE CHAT ROUTES (UNIFIED USERS TABLE)
# ============================================

# ✅ FIXED: live_chat redirect uses users_chat instead of users
@app.route('/live-chat')
@login_required
def live_chat():
    # Get user_id from session, not Flask-Login current_user
    user_id = session.get('user_id')
    if not user_id:
        flash('Please login first', 'error')
        return redirect(url_for('login'))
    
    # Get user info from session or DB for display
    user_name = session.get('full_name', 'User')
    
    try:
        # Fetch other users for the sidebar - excluding self
        ...
        return render_template('live.chat.html', 
                             current_user={'id': user_id, 'username': user_name},
                             users=users,
...
def live_users():
    ...
    try:
        ...
        return render_template('live.users.html', 
                             users=users, 
                             current_user={'id': user_id, 'username': session.get('full_name', 'User')},
...
# ============================================
# ✅ GROUP VIDEO CALL ROUTES (UNIFIED USERS TABLE)
# ============================================
...
def create_group_call():
    ...
    try:
        ...
        active_group_calls[call_id] = {
            'host_id': user_id,
            'host_name': session.get('full_name', 'User'),
            'created_at': get_utc_time(),
            'is_active': True,
            'participant_count': 1
        }
...
# ============================================
# ✅ SOCKETIO EVENTS (UNIFIED USERS TABLE) - FIXED: session-based auth
# ============================================
...
# ✅ UPDATED: connect uses session instead of Flask-Login
@socketio.on('connect')
def handle_connect():
...
def handle_join_group_call_room(data):
    ...
    if call_id:
        ...
        join_room(room_name)
        logger.info(f"User {session.get('full_name', 'User')} joined group call room {call_id}")
...
# ============================================
# ✅ ADDED: /users redirect route for backward compatibility
# ============================================

@app.route('/users')
@login_required
def users_redirect():
    """Redirect /users to /users-chat for backward compatibility"""
    # Preserve query parameters if any
    query_string = request.query_string.decode('utf-8') if request.query_string else ''
    if query_string:
        return redirect(url_for('users_chat') + '?' + query_string)
    return redirect(url_for('users_chat'))
...
def cart_count():
    ...
    try:
        ...
        logger.error(f"Cart count error: {e}")
        return jsonify({'count': 0})
...
if __name__ == '__main__':
    ...
    if not is_render:
        ...
        print(f"✅ FIXED: live_chat and view_user_profile redirect to users_chat")
        print(f"✅ FIXED: get_live_messages has global declaration")
        print(f"✅ FIXED: Added /users redirect route")
        print(f"✅ FIXED: Added /api/cart/count endpoint")
        ...
        print("✅ FIXED: live_chat and view_user_profile redirect to users_chat")
        print("✅ FIXED: get_live_messages has global declaration")
        print("✅ FIXED: Added /users redirect route")
        print("✅ FIXED: Added /api/cart/count endpoint")
