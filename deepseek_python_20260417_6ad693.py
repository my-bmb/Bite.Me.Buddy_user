...
# ✅ ADDED: Complete Referral System (₹30 reward after ₹1000 spend)
# ✅ ADDED: Wallet System with withdrawal requests
# ✅ ADDED: Referral stats in profile page
...
def process_referral_reward(user_id, amount_spent):
    ...
    try:
        ...
        new_total = current_total + amount_spent
        
        # Update user's total_spent
        supabase_execute('users', 'update', 
                        data={'total_spent': new_total, 'updated_at': datetime.now().isoformat()},
                        conditions={'id': user_id}, use_admin=True)
        
        print(f"💰 User {user_id} total_spent updated: {current_total} → {new_total}")
        
        # Check reward condition: total_spent >= 1000 AND reward_given == False
        if new_total >= 1000 and not user.get('reward_given', False):
            referral_mobile = user.get('referral_mobile')
            
            if referral_mobile:
                print(f"🎁 User {user_id} reached ₹1000 spend! Checking referrer: {referral_mobile}")
                
                # Find referrer by mobile number
                referrers = supabase_execute('users', 'select', 
                                            conditions={'mobile': referral_mobile}, 
                                            use_admin=True)
...
# ============================================
# ✅ AUTHENTICATION ROUTES - WITH REFERRAL SUPPORT
# ============================================
...
def register():
    if request.method == 'POST':
        ...
        # Validate referral mobile if provided
        if referral_mobile:
...
# ============================================
# ✅ OPTIMIZED DASHBOARD ROUTE (ORIGINAL - KEEP FOR BACKWARD COMPATIBILITY)
# ============================================
...
# ============================================
# ✅ OPTIMIZED SERVICES ROUTE (ORIGINAL)
# ============================================
...
# ============================================
# ✅ OPTIMIZED GOODS ROUTE (ORIGINAL)
# ============================================
...
# ============================================
# ✅ WORKING CART ROUTE (FROM PREVIOUS FILE)
# ============================================
...
# ============================================
# ✅ ORDER DETAILS ROUTE - UPDATED FROM SECOND FILE
# ============================================
        ...
        try:
            ...
            if delivery_lat is not None and delivery_lng is not None:
                try:
                    ...
                    supabase.table('users').update(update_user_data).eq('id', session['user_id']).execute()
            ...
            if payment_mode == 'online':
                ...
                reward_result = process_referral_reward(session['user_id'], total_amount)
...
# ============================================
# ✅ PROFILE ROUTE WITH REFERRAL AND WALLET
# ============================================
...
def profile():
    ...
    # GET REQUEST - Show profile with referral stats
    try:
        user_id = session['user_id']
        ...
        if users:
            ...
            total_earned = reward_transactions and sum(tx.get('amount', 0) for tx in reward_transactions) or 0
            total_withdrawn = withdrawals and sum(w.get('amount', 0) for w in withdrawals if w.get('status') == 'completed') or 0
            pending_withdrawal = withdrawals and sum(w.get('amount', 0) for w in withdrawals if w.get('status') == 'pending') or 0
            
            # Generate referral link
            base_url = request.url_root.rstrip('/')
            referral_link = f"{base_url}/register?ref={session.get('phone')}"
    ...
    return render_template('profile.html', 
                         ...
                         total_earned=0, total_withdrawn=0, pending_withdrawal=0,
                         referred_users=[], reward_transactions=[], withdrawals=[],
                         referral_link='')
...
def request_withdrawal():
    ...
    try:
        ...
        result = create_withdrawal_request(session['user_id'], amount, withdrawal_method, bank_details, upi_id)
...
def get_wallet_balance():
    ...
    try:
        users = supabase_execute('users', 'select', conditions={'id': session['user_id']}, use_admin=True)
        if users:
            return jsonify({
                'success': True,
...
def cart_ultrafast():
    ...
    try:
        ...
        cart_items_db = supabase_execute('cart', 'select', conditions={'user_id': session['user_id']})
