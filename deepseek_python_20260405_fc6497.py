...
# ============================================
# ✅ TRENDING ITEMS FUNCTION - UPDATED WITH JSONB & STRING HANDLING + ADMIN BYPASS
# ============================================
...
def get_trending_items(limit=10):
    ...
    Uses ADMIN client to bypass RLS for trending calculation
    """
    try:
        from datetime import datetime, timedelta
        from dateutil import parser
        ...
        # ✅ CRITICAL FIX: Use ADMIN client to bypass RLS
        orders = supabase_execute(
            'orders',
            'select',
            conditions={},
            use_admin=True  # ← Admin bypass for trending
        )
        ...
        print(f"📊 [TRENDING] Total orders found (via admin): {len(orders)}")
...
# ============================================
# ✅ DEBUG ROUTES FOR TRENDING - UNLIMITED DEBUGGING
# ============================================
...
def debug_trending_check():
    ...
    try:
        # Get trending items
        trending = get_trending_items(limit=10)
        
        # Get all orders (admin bypass)
            ...
            for order in all_orders[:3]:
                if order.get('items'):
                    sample_order = {
                        ...
                        'items_raw_type': str(type(order.get('items'))),
                        'items_raw': str(order.get('items'))[:200] if order.get('items') else None
                    }
                    try:
                        if isinstance(order.get('items'), str):
                            sample_items_parsed = json.loads(order.get('items'))
                        else:
                            sample_items_parsed = order.get('items')
                        sample_order['items_parsed'] = sample_items_parsed
                        break
                    except:
                        pass
...
def debug_orders_direct():
    ...
    try:
        # Get orders using admin client
        orders = supabase_execute('orders', 'select', conditions={}, use_admin=True)
        
        # Get orders using regular client (for comparison)
...
def debug_rls_status():
    ...
    try:
        # Try to get orders with regular client (RLS enforced)
        orders_regular = supabase_execute('orders', 'select', conditions={}, use_admin=False)
        
        # Try to get orders with admin client (bypass RLS)
        orders_admin = supabase_execute('orders', 'select', conditions={}, use_admin=True)
        
        # Try to get current user info
        current_user_id = session.get('user_id')
        
        # Try to get user's own orders
...
def debug_routes():
    ...
    <body>
        ...
        <table>
            <thead>
                <tr><th>#</th><th>Endpoint</th><th>URL</th><th>Methods</th></tr>
            </thead>
            <tbody>
    """
    ...
    html += """
        ...
        <a href="/debug-orders-direct">📦 Orders Direct</a> |
        <a href="/debug-items-parsing">📝 Items Parsing</a> |
        <a href="/debug-rls-status">🔒 RLS Status</a> |
        <a href="/debug-create-test-order">➕ Create Test Order</a>
    </body>
    </html>
    """
