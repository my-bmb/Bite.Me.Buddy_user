# app.py - COMPLETE MERGED VERSION 
# Profile time formatting from app36.py (WORKING)
# Order time formatting from app37.py (UTC to IST conversion)
import os
...
def to_ist(datetime_obj):
    ...
    # ✅ Agar string hai to datetime mein convert karo
...
def format_ist_datetime(datetime_obj, format_str="%d %b %Y, %I:%M %p"):
    ...
    # ✅ Agar string hai to pehle convert karo
    if isinstance(datetime_obj, str):
        try:
            datetime_obj = parser.parse(datetime_obj)
            print(f"✅ Parsed string: {datetime_obj}")
        except:
            return datetime_obj
    ...
    try:
        ...
        if ist_time and hasattr(ist_time, 'strftime'):
            formatted = ist_time.strftime(format_str)
        ...
        try:
            ...
            if users and len(users) > 0:
                ...
                if check_password_hash(user['password'], password):
                    ...
                    # ✅ EXACT app36.py CODE - WORKING VERSION FOR PROFILE DATE
                    if user.get('created_at'):
                        try:
                            # Format as "12 Feb 2026" for profile
                            formatted_date = format_ist_datetime(user['created_at'], "%d %b %Y")
                            session['created_at'] = formatted_date
                            print(f"✅ Profile date set: {formatted_date}")  # DEBUG
                        except Exception as e:
                            print(f"⚠️ Date error: {e}")
                            # Fallback - app36.py style
                            if user['created_at']:
...
# ============================================
# ✅ ORDER HISTORY ROUTE - SUPABASE (FROM app37.py)
# ============================================
...
def order_history():
    try:
        ...
        for index, order in enumerate(orders_data):
            ...
            # ✅ FROM app37.py: Proper UTC to IST conversion for orders
...
# ============================================
# ✅ ORDER DETAILS ROUTE - SUPABASE (FROM app37.py)
# ============================================
...
def order_details(order_id):
    ...
    try:
        ...
        # ✅ FROM app37.py: Convert dates to IST properly
        if order.get('order_date'):
            try:
                ist_time = to_ist(order['order_date'])
                order['order_date_formatted'] = ist_time.strftime("%d %b %Y, %I:%M %p") if ist_time else "Date error"
...
# ============================================
# ✅ PROFILE ROUTES - SUPABASE (FROM app36.py - WORKING)
# ============================================
