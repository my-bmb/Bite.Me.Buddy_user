# app.py - COMPLETE UPDATED VERSION WITH DASHBOARD (DISCOUNTS + COLLECTIONS)
import os
...
def dashboard():
    ...
    try:
        # 1. GET TOP DISCOUNT ITEMS (Services + Menu Items combined, sorted by discount)
        top_discount_items = []
        ...
        # Get menu items with discount > 0
        menu_items = supabase_execute('menu_items', 'select', conditions={'status': 'active'}, use_admin=False)
        print(f"📊 [DASHBOARD] Found {len(menu_items) if menu_items else 0} total menu items")
            ...
            for item in menu_items:
                ...
                if discount > 0:
                    ...
                    top_discount_items.append({
                        ...
                        'url': url_for('menu_item_details', item_id=item['id'])
                    })
            print(f"✅ [DASHBOARD] Menu items with discount: {menu_count}")
        else:
            print("⚠️ [DASHBOARD] No menu items found in database!")
        ...
        # 2. GET NEW ARRIVALS (latest 10 services + menu items)
        new_arrivals = []
        ...
        if menu_items:
            latest_menu = sorted(menu_items, key=lambda x: x.get('created_at', ''), reverse=True)[:5]
        ...
        # 4. GET MENU COLLECTIONS
        menu_collections = supabase_execute('menu_collections', 'select', conditions={'status': 'active'}, use_admin=False)
        if menu_collections:
            menu_collections = sorted(menu_collections, key=lambda x: x.get('position', 0))
            print(f"📁 [DASHBOARD] Menu collections: {len(menu_collections)}")
        else:
            menu_collections = []
        ...
        # 6. GET ALL MENU ITEMS (for All Products section)
        all_menu_items = supabase_execute('menu_items', 'select', conditions={'status': 'active'}, use_admin=False)
        if all_menu_items:
            all_menu_items = sorted(all_menu_items, key=lambda x: x.get('position', 0))
            for item in all_menu_items:
                if not item.get('photo'):
                    item['photo'] = "https://res.cloudinary.com/demo/image/upload/v1633427556/sample_food.jpg"
            print(f"📦 [DASHBOARD] All products: {len(all_menu_items)}")
        else:
            all_menu_items = []
        ...
        for item in cart_items[:3]:
            if item['item_type'] == 'service':
                ...
                details = supabase_execute('menu_items', 'select', conditions={'id': item['item_id']})
...
def get_trending_items(limit=10):
    ...
    Returns list of trending services and menu items
    """
    try:
                    ...
                    for item in items:
                        ...
                        if key not in item_count:
                            ...
                            item_details[key] = {
                                ...
                                'photo': item_photo,
                                'price': item_price,
                                'url': url_for('service_details', service_id=item_id) if item_type == 'service' else url_for('menu_item_details', item_id=item_id)
                            }
