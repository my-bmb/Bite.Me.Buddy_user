# app.py - COMPLETE UPDATED VERSION WITH DASHBOARD (DISCOUNTS + COLLECTIONS) - GOODS VERSION
import os
...
GOODS_FOLDER = "goods_items"  # Changed from MENU_FOLDER
...
def get_all_internal_urls():
    ...
    main_routes = ['dashboard', 'services', 'goods', 'cart', 'orders', 'profile']  # Changed from 'menu' to 'goods'
    ...
    try:
        ...
        goods_collections = supabase_execute('goods_collections', 'select', conditions={'status': 'active'}, use_admin=False)  # Changed from menu_collections
        for collection in goods_collections[:5]:  # Prefetch first 5 collections
            try:
                urls.append(url_for('goods_collection_categories', collection_id=collection['id']))  # Changed
            except:
                pass
...
def get_goods_hierarchy():
    ...
    try:
        ...
        collections = supabase_execute('goods_collections', 'select', conditions={'status': 'active'}, use_admin=False)  # Changed
        collections = sorted(collections, key=lambda x: x.get('position', 0)) if collections else []
        
        # For each collection, get its categories and items
        for collection in collections:
            # Get categories for this collection
            categories = supabase_execute('goods_categories', 'select',  # Changed
                                         conditions={'collection_id': collection['id'], 'status': 'active'},
                                         use_admin=False)
            categories = sorted(categories, key=lambda x: x.get('position', 0)) if categories else []
            
            # For each category, get its goods items
            for category in categories:
                items_list = supabase_execute('goods_items', 'select',  # Changed
                                           conditions={'category_id': category['id'], 'status': 'active'},
                                           use_admin=False)
                ...
                for item in items_list:
                    if not item.get('photo'):
                        item['photo'] = "https://res.cloudinary.com/demo/image/upload/v1633427556/sample_goods.jpg"  # Changed
                ...
                # Add default category photo if not present
                if not category.get('category_photo'):
                    category['category_photo'] = "https://res.cloudinary.com/demo/image/upload/v1633427556/default_goods_category.jpg"  # Changed
            ...
            # Add default collection photo if not present
            if not collection.get('collection_photo'):
                collection['collection_photo'] = "https://res.cloudinary.com/demo/image/upload/v1633427556/default_goods_collection.jpg"  # Changed
...
def dashboard():
    ...
    try:
        # 1. GET TOP DISCOUNT ITEMS (Services + Goods Items combined, sorted by discount)
        top_discount_items = []
        
        # Get services with discount > 0
        ...
        # Get goods items with discount > 0
        goods_items = supabase_execute('goods_items', 'select', conditions={'status': 'active'}, use_admin=False)  # Changed
            ...
            for item in goods_items:
                ...
                if discount > 0:
                    ...
                    top_discount_items.append({
                        'id': item['id'],
                        'name': item['name'],
                        'type': 'goods',  # Changed from 'menu'
                        'photo': item.get('photo', 'https://res.cloudinary.com/demo/image/upload/v1633427556/sample_goods.jpg'),  # Changed
                        'price': float(item.get('price', 0)),
                        'discount': discount,
                        'final_price': float(item.get('final_price', 0)),
                        'url': url_for('goods_item_details', item_id=item['id'])  # Changed
                    })
        ...
        # 2. GET NEW ARRIVALS (latest 10 services + goods items)
        new_arrivals = []
        ...
        # Sort new arrivals by date (newest first) and take top 12
        ...
        # 4. GET GOODS COLLECTIONS
        goods_collections = supabase_execute('goods_collections', 'select', conditions={'status': 'active'}, use_admin=False)  # Changed
        ...
        # 5. GET ALL SERVICES (for All Services section)
        ...
        # 6. GET ALL GOODS ITEMS (for All Products section)
        all_goods_items = supabase_execute('goods_items', 'select', conditions={'status': 'active'}, use_admin=False)  # Changed
        if all_goods_items:
        ...
        # 7. GET TRENDING ITEMS (from orders table)
        ...
        # Get cart items with details for preview
        cart_items_preview = []
        cart_total = 0
        for item in cart_items[:3]:
            if item['item_type'] == 'service':
                ...
                details = supabase_execute('goods_items', 'select', conditions={'id': item['item_id']})  # Changed
        ...
        return render_template('dashboard.html',
                             ...
                             goods_collections=goods_collections,  # Changed
                             all_services=all_services,
                             all_goods_items=all_goods_items,  # Changed
        ...
        return render_template('dashboard.html',
                             ...
                             new_arrivals=[],
                             service_collections=[],
                             goods_collections=[],  # Changed
                             all_services=[],
                             all_goods_items=[],  # Changed
                             trending_items=[],
                             cart_count=0,
                             cart_items=[],
                             cart_total=0,
                    ...
                    for item in items:
                        ...
                        if key not in item_count:
                            ...
                            if item_type == 'service':
                                ...
                                url = url_for('goods_item_details', item_id=item_id)  # Changed
...
def get_goods_category_items(category_id):
    ...
    try:
        # Fetch all goods items for this category
...
def goods():
    ...
    try:
        # Get full goods hierarchy using the new helper function
...
def goods_category_items(category_id):
    ...
    try:
        ...
        # Get all goods items for this category
        goods_items = supabase_execute(
            'goods_items',
            'select',
            conditions={'category_id': category_id, 'status': 'active'}
...
def goods_item_details(item_id):
    ...
    try:
        # Get goods item details with ALL columns
        goods_items = supabase_execute(
            'goods_items',
            'select',
        ...
        for item in cart_items_db:
            if item['item_type'] == 'service':
                ...
                if goods_item:
                    ...
                    item_price = float(goods_item['final_price'])
                    item_description = goods_item.get('description', '')
                    db_photo = goods_item.get('photo', '')
...
def add_to_cart():
    ...
    if item_type not in ['service', 'goods']:  # Changed from 'menu' to 'goods'
...
def debug_data():
    ...
    try:
        ...
        column_info = {}
        tables = ['service_collections', 'service_categories', 'services', 'goods_collections', 'goods_categories', 'goods_items']  # Changed
