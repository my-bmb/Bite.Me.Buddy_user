    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                cart_data = cur.fetchall()  # ✅ fetchall() call kiya
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                result = cur.fetchone()  # ✅ fetchone() call kiya
                if not result:
...
def order_history():
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                # Get orders - fetchall() CALL KAREIN
                cur.execute("""
                    SELECT * FROM orders 
                    WHERE user_id = %s 
                ...
                orders = cur.fetchall()  # ✅ YEH LINE IMPORTANT HAI - fetchall() add kiya
