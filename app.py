# app.py - COMPLETE UPDATED VERSION WITH DASHBOARD DATA FOR ALL 13 TABLES
import os
...
def get_all_internal_urls():
    ...
    main_routes = ['dashboard', 'services', 'menu', 'cart', 'orders', 'profile']
    ...
    try:
        ...
        for collection in collections[:5]:  # Prefetch first 5 collections
            try:
                urls.append(url_for('service_collection_categories', collection_id=collection['id']))
            except:
                pass
        
        # Get all menu collections
        menu_collections = supabase_execute('menu_collections', 'select', conditions={'status': 'active'}, use_admin=False)
        for collection in menu_collections[:5]:  # Prefetch first 5 collections
            try:
                urls.append(url_for('menu_collection_categories', collection_id=collection['id']))
            except:
                pass
...
def utility_processor():
    ...
    # ✅ Add prefetch URLs to all templates
    ...
    return dict(
        ...
        prefetch_urls=prefetch_urls  # Add prefetch URLs to all templates
    )
...
@login_required
def dashboard():
    """Dashboard showing all 13 tables data"""
    print("\n🔍 [DASHBOARD] Fetching data from all 13 tables...")
    ...
    try:
        ...
        dashboard_data['users'] = {
            ...
            'data': users[:10] if users else []
        }
        print(f"📊 [DASHBOARD] users: {dashboard_data['users']['count']} records")
        
        # 2. service_collections table
        service_collections = supabase_execute('service_collections', 'select', use_admin=False)
        dashboard_data['service_collections'] = {
            'count': len(service_collections) if service_collections else 0,
            'data': service_collections if service_collections else []
        }
        print(f"📊 [DASHBOARD] service_collections: {dashboard_data['service_collections']['count']} records")
        
        # 3. service_categories table
        service_categories = supabase_execute('service_categories', 'select', use_admin=False)
        dashboard_data['service_categories'] = {
            'count': len(service_categories) if service_categories else 0,
            'data': service_categories if service_categories else []
        }
        print(f"📊 [DASHBOARD] service_categories: {dashboard_data['service_categories']['count']} records")
        
        # 4. services table
        services = supabase_execute('services', 'select', use_admin=False)
        dashboard_data['services'] = {
            'count': len(services) if services else 0,
            'data': services[:10] if services else []
        }
        print(f"📊 [DASHBOARD] services: {dashboard_data['services']['count']} records")
        
        # 5. menu_collections table
        menu_collections = supabase_execute('menu_collections', 'select', use_admin=False)
        dashboard_data['menu_collections'] = {
            'count': len(menu_collections) if menu_collections else 0,
            'data': menu_collections if menu_collections else []
        }
        print(f"📊 [DASHBOARD] menu_collections: {dashboard_data['menu_collections']['count']} records")
        
        # 6. menu_categories table
        menu_categories = supabase_execute('menu_categories', 'select', use_admin=False)
        dashboard_data['menu_categories'] = {
            'count': len(menu_categories) if menu_categories else 0,
            'data': menu_categories if menu_categories else []
        }
        print(f"📊 [DASHBOARD] menu_categories: {dashboard_data['menu_categories']['count']} records")
        
        # 7. menu_items table
        menu_items = supabase_execute('menu_items', 'select', use_admin=False)
        dashboard_data['menu_items'] = {
            'count': len(menu_items) if menu_items else 0,
            'data': menu_items[:10] if menu_items else []
        }
        print(f"📊 [DASHBOARD] menu_items: {dashboard_data['menu_items']['count']} records")
        
        # 8. cart table (current user only)
        ...
        print(f"📊 [DASHBOARD] cart: {dashboard_data['cart']['count']} records")
        
        # 9. orders table
        orders = supabase_execute('orders', 'select', use_admin=False)
        dashboard_data['orders'] = {
            'count': len(orders) if orders else 0,
            'data': orders[:10] if orders else []
        }
        print(f"📊 [DASHBOARD] orders: {dashboard_data['orders']['count']} records")
        
        # 10. payments table
        payments = supabase_execute('payments', 'select', use_admin=False)
        dashboard_data['payments'] = {
            'count': len(payments) if payments else 0,
            'data': payments[:10] if payments else []
        }
        print(f"📊 [DASHBOARD] payments: {dashboard_data['payments']['count']} records")
        
        # 11. addresses table
        addresses = supabase_execute('addresses', 'select', use_admin=False)
        dashboard_data['addresses'] = {
            'count': len(addresses) if addresses else 0,
            'data': addresses[:10] if addresses else []
        }
        print(f"📊 [DASHBOARD] addresses: {dashboard_data['addresses']['count']} records")
        
        # 12. notifications table
        notifications = supabase_execute('notifications', 'select', use_admin=False)
        dashboard_data['notifications'] = {
            'count': len(notifications) if notifications else 0,
            'data': notifications[:10] if notifications else []
        }
        print(f"📊 [DASHBOARD] notifications: {dashboard_data['notifications']['count']} records")
        
        # 13. order_items table
        order_items = supabase_execute('order_items', 'select', use_admin=False)
        dashboard_data['order_items'] = {
            'count': len(order_items) if order_items else 0,
            'data': order_items[:10] if order_items else []
        }
        print(f"📊 [DASHBOARD] order_items: {dashboard_data['order_items']['count']} records")
        
        # Calculate total records
        total_records = sum([
            dashboard_data['users']['count'],
            dashboard_data['service_collections']['count'],
            dashboard_data['service_categories']['count'],
            dashboard_data['services']['count'],
            dashboard_data['menu_collections']['count'],
            dashboard_data['menu_categories']['count'],
            dashboard_data['menu_items']['count'],
            dashboard_data['cart']['count'],
            dashboard_data['orders']['count'],
            dashboard_data['payments']['count'],
            dashboard_data['addresses']['count'],
            dashboard_data['notifications']['count'],
            dashboard_data['order_items']['count']
        ])
        
        print(f"\n✅ [DASHBOARD] TOTAL RECORDS: {total_records} across 13 tables")
...
def service_category_services(category_id):
    ...
    try:
        ...
        # ✅ GET ALL CATEGORIES FOR SIDEBAR NAVIGATION (COMPLETE TABLE)
        ...
        if all_categories:
            ...
            # Add default photos for categories
...
# ============================================
# ✅ API ENDPOINTS - GET CATEGORY SERVICES (SERVICES & MENU)
# ============================================
...
@app.route('/get_menu_category_items/<int:category_id>')
@login_required
def get_menu_category_items(category_id):
    """API endpoint to get all menu items for a specific menu category"""
    print(f"\n🔍 [API] Fetching menu items for category ID: {category_id}")
    try:
        ...
        for item in menu_items:
            items_data.append({
                'id': item['id'],
                'name': item.get('name', ''),
                'photo': item.get('photo', 'https://res.cloudinary.com/demo/image/upload/v1633427556/sample_food.jpg'),
        ...
        print(f"✅ [API] Found {len(items_data)} menu items for category {category_id}")
        return jsonify({'success': True, 'items': items_data})
        
    except Exception as e:
        print(f"❌ [API] Error fetching menu category items: {e}")
        traceback.print_exc()
...
def service_details(service_id):
    ...
    try:
        ...
        # ✅ GET ALL SERVICES FROM CURRENT CATEGORY (for "You Might Also Like" section)
        category_services = []
        if category:
            ...
            # Add photos for category services
        ...
        # ✅ GET ALL CATEGORIES WITH THEIR SERVICES (for displaying all categories with their services)
...
def menu_category_items(category_id):
    ...
    try:
        ...
        if all_categories:
            all_categories = sorted(all_categories, key=lambda x: x.get('position', 0))
            print(f"📊 [MENU-CATEGORY-ITEMS] Found {len(all_categories)} total categories for sidebar")
...
# ============================================
# ✅ MENU ITEM DETAIL ROUTE - WITH CATEGORY ITEMS
# ============================================
...
def menu_item_details(item_id):
    ...
    try:
        ...
        # ✅ GET ALL ITEMS FROM CURRENT CATEGORY (for "You Might Also Like" section)
        category_items = []
        if category:
            ...
            # Add photos for category items
            for cat_item in category_items:
                if not cat_item.get('photo') or cat_item['photo'] == '':
                    cat_item['photo'] = "https://res.cloudinary.com/demo/image/upload/v1633427556/sample_food.jpg"
        
        print(f"📊 [MENU-ITEM] Found {len(category_items)} other items in same category")
        
        # ✅ GET ALL CATEGORIES WITH THEIR ITEMS (for displaying all categories with their items)
        all_categories_list = supabase_execute(
            'menu_categories',
            'select',
            conditions={'status': 'active'}
        ...
        print(f"📊 [MENU-ITEM] Found {len(all_categories_with_items)} categories with their items")
...
# ============================================
# ✅ PREFETCH API ENDPOINT - FOR DYNAMIC PRELOADING
# ============================================
