# app.py - COMPLETE UPDATED VERSION WITH RAZORPAY PAYMENT INTEGRATION
import os
...
def generate_customer_order_no(order_id, user_id):
    """
    Generate customer-friendly order number like "ORD001"
    """
    try:
        with get_db_connection() as conn:
            ...
            # Format: ORD followed by sequential number
            return f"ORD{total_orders:03d}"
        ...
        with get_db_connection() as conn:
            ...
            with conn.cursor() as cur:
                ...
                if not users_table_exists:
                    ...
                    # ✅ 5. ORDERS TABLE (Added order_no column)
                    cur.execute("""
                        CREATE TABLE IF NOT EXISTS orders (
                            order_id SERIAL PRIMARY KEY,
                            order_no VARCHAR(50),  -- ✅ Customer friendly order number
                            user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
                    ...
                    # Check if orders table has order_no column
                    cur.execute("""
                        SELECT column_name 
                        FROM information_schema.columns 
                    ...
                    """)
                    if not cur.fetchone():
                        print("⚠️ Adding missing 'order_no' column to orders table...")
                        cur.execute("ALTER TABLE orders ADD COLUMN order_no VARCHAR(50)")
...
# ============================================
# ✅ CHECKOUT & ORDERS ROUTES (UPDATED WITH RAZORPAY)
# ============================================
        ...
        try:
            with get_db_connection() as conn:
                ...
                if payment_mode == 'online':
                    ...
                    order_no = generate_customer_order_no(None, session['user_id'])
                    
                    # First create order in our database with 'pending_payment' status
                    cur.execute("""
                        INSERT INTO orders 
                        (order_no, user_id, user_name, user_email, user_phone, user_address, 
                         items, total_amount, payment_mode, delivery_location, status)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 'pending_payment')
                        RETURNING order_id
                    """, (order_no, session['user_id'], session.get('full_name'), session.get('email'), 
                    ...
                    """, (order_id, session['user_id'], total_amount, payment_mode))
...
def payment_page(order_id):
    """Payment page for online payment with order_no"""
    try:
        with get_db_connection() as conn:
            cur = conn.cursor()
            
            # Get order details with order_no
            cur.execute("""
                SELECT 
                    o.order_id,
                    o.order_no,
            ...
            # ✅ Use order_no in payment page
            order_no = order['order_no'] or f"ORD{order_id:03d}"
            
            print(f"✅ [PAYMENT_PAGE] Order ID: {order_id}, Order No: {order_no}")
            
            return render_template('payment.html',
                                 order_no=order_no,  # ✅ Yahan order_no pass kar rahe hain
...
def create_razorpay_order():
    ...
    try:
        ...
        # Get order_no from database
        order_no = ""
        try:
            with get_db_connection() as conn:
                cur = conn.cursor()
                cur.execute("SELECT order_no FROM orders WHERE order_id = %s", (order_id,))
                result = cur.fetchone()
                if result and result['order_no']:
                    order_no = result['order_no']
                else:
                    order_no = f"ORD{order_id:03d}"
                cur.close()
        except Exception as e:
            print(f"⚠️ Error getting order_no: {e}")
            order_no = f"ORD{order_id:03d}"
        ...
        order_data = {
            ...
            'notes': {
                'order_id': order_id,
                'order_no': order_no,  # ✅ Add order_no in notes
...
def verify_razorpay_payment():
    ...
    try:
        ...
        print(f"✅ [RAZORPAY] Database updated for order #{order_id}")
        
        # Use order_no for redirect if available
        redirect_order_no = order_no if order_no else order_id
        
        return jsonify({
            'success': True,
            'payment_id': razorpay_payment_id,
            'order_no': redirect_order_no,
        ...
        with get_db_connection() as conn:
            ...
            if payment:
                return jsonify({
                    ...
                    'payment_date': format_ist_datetime(payment['payment_date']) if payment['payment_date'] else None
                })
            else:
                return jsonify({
                    'success': False,
...
# ============================================
# ✅ ORDER HISTORY ROUTE (UPDATED WITH order_no)
# ============================================
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                for order in orders_data:
                    ...
                    # Use order_no from database or fallback to generated
                    order_no = order.get('order_no')
                    if not order_no:
                        # Generate order_no if not present
                        order_no = f"ORD{order['order_id']:03d}"
                    
                    # Add to orders list
                    orders_list.append({
                        'order_id': order['order_id'],
                        'order_no': order_no,  # ✅ Use order_no from database
        ...
        if orders_list:
            ...
            for order in orders_list:
                print(f"  - Order ID: {order['order_id']}, Order No: {order['order_no']}, Date: {order['order_date_formatted']}")
...
# ============================================
# ✅ ORDER DETAILS ROUTE (UPDATED WITH order_no)
# ============================================
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                # Get order_no from database or generate
                order_no = order.get('order_no')
                if not order_no:
                    ...
                    result = cur.fetchone()
                    total_orders = result['total_orders'] if result else 0
                    order_no = f"ORD{total_orders:03d}"
                ...
                # If no items from JSON, use order_items table
                ...
                # If still no items, create dummy
                if not items_list:
                    items_list.append({
                ...
                # Add order_no to order dict
                order['order_no'] = order_no
                
                print(f"✅ [ORDER_DETAILS] Final - Order ID: {order_id}, Order No: {order['order_no']}")
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    SELECT order_id, order_no, user_id, user_name, order_date 
                    FROM orders 
                    WHERE user_id = %s 
                ...
                <html>
                <head>
                    <title>Orders Debug Information</title>
                    <style>
                ...
                <body>
                    <h2>Orders Debug Information</h2>
                    ...
                    <div class="stats">
                        ...
                        <p><strong>Total Users:</strong> {len(all_users)}</p>
                    </div>
                ...
                for index, order in enumerate(user_orders):
                    html += f"""
                        <tr>
                            ...
                            <td><strong>{order['order_no'] or 'N/A'}</strong></td>
                            <td>{order['user_id']}</td>
...
def utility_processor():
    ...
    # ✅ Add Razorpay key and order_no generator to all templates
    return dict(
        get_user_location=get_user_friendly_location,
        ist_now=ist_now,
        to_ist=to_ist,
        format_ist_time=format_ist_time,
        razorpay_key_id=RAZORPAY_KEY_ID,  # ✅ Sabhi templates mein available hoga
        generate_order_no=generate_customer_order_no  # ✅ Order number generator
    )

# ============================================
# ✅ FIX ORDER_NOS FOR EXISTING ORDERS
# ============================================

@app.route('/fix-order-numbers')
@login_required
def fix_order_numbers():
    """Fix order numbers for existing orders"""
    try:
        with get_db_connection() as conn:
            cur = conn.cursor()
            ...
            for user in users:
                ...
                for index, order in enumerate(user_orders):
                    ...
                    results.append(f"User {user_id}: Order {order['order_id']} → {order_no}")
            
            conn.commit()
            cur.close()
            
            return f"""
            <h2>Order Numbers Fixed</h2>
            <p>Total users: {len(users)}</p>
            <p>Updated {len(results)} orders</p>
            <hr>
            {'<br>'.join(results[:50])}
            {f'<p>... and {len(results)-50} more</p>' if len(results) > 50 else ''}
            <hr>
            <p><a href="/order_history">← Back to Order History</a></p>
            """
