...
# ============================================
# ✅ OPTIMIZED CART HELPER
# ============================================

def get_cart_with_details_fast(user_id):
    """Get cart with ALL item details in MINIMAL queries (2-3 only)"""
    try:
        # Query 1: Get cart items
        cart_result = supabase.table('cart')\
            .select('*')\
            .eq('user_id', user_id)\
            .execute()
        ...
        # Query 2 & 3: Batch fetch all services and goods
        ...
        for item in cart_items:
            ...
            if details:
                item_total = float(details['final_price']) * item['quantity']
                total += item_total
        ...
        print(f"❌ [get_cart_with_details_fast] Error: {e}")
        traceback.print_exc()
        return [], 0
...
# ============================================
# ✅ FIXED CART ROUTE (NOW WORKING)
# ============================================

@app.route('/cart')
@login_required
def cart():
    """Display cart - FIXED VERSION with better error handling"""
    print("\n🔍 [CART FIXED] Fetching cart...")
    start_time = time.time()
    ...
    try:
        ...
        try:
            ...
            cart_items_db = result.data if result.data else []
            print(f"  Direct fetch found: {len(cart_items_db)} items")
        except Exception as e:
            print(f"  Direct fetch error: {e}")
            cart_items_db = supabase_execute('cart', 'select', conditions={'user_id': user_id}, use_admin=True) or []
            print(f"  Admin fetch found: {len(cart_items_db)} items")
        
        if not cart_items_db:
            print("  Cart is empty")
            return render_template('cart.html', cart_items=[], total_amount=0, active_tab='cart')
        
        # Collect all service and goods IDs for batch fetching
        ...
        print(f"  Goods IDs: {goods_ids}")
        
        # Batch fetch all services
        services_dict = {}
        if service_ids:
            try:
                services_result = supabase.table('services').select('*').in_('id', service_ids).execute()
                for service in (services_result.data or []):
                    services_dict[service['id']] = service
                print(f"  Fetched {len(services_dict)} services")
            except Exception as e:
                print(f"  Error fetching services: {e}")
        
        # Batch fetch all goods
        goods_dict = {}
        if goods_ids:
            try:
                goods_result = supabase.table('goods_items').select('*').in_('id', goods_ids).execute()
                for goods in (goods_result.data or []):
                    goods_dict[goods['id']] = goods
                print(f"  Fetched {len(goods_dict)} goods items")
            except Exception as e:
        ...
        for item in cart_items_db:
            ...
            if item_type == 'service':
                details = services_dict.get(item_id)
            else:
                details = goods_dict.get(item_id)
            ...
            if details:
                ...
                print(f"  Added: {details['name']} x{quantity} = ₹{item_total}")
            else:
                print(f"  WARNING: {item_type} with id {item_id} not found in database")
        
        elapsed = time.time() - start_time
        print(f"✅ [CART FIXED] Loaded {len(cart_items)} items in {elapsed:.2f}s")
...
# ============================================
# ✅ OPTIMIZED ORDER DETAILS ROUTE
# ============================================

@app.route('/order/<int:order_id>')
@login_required
def order_details(order_id):
    """View detailed order information - OPTIMIZED"""
    print(f"\n🔍 [ORDER_DETAILS OPTIMIZED] Fetching order {order_id}...")
    start_time = time.time()
    ...
    try:
        ...
        elapsed = time.time() - start_time
        print(f"✅ [ORDER_DETAILS OPTIMIZED] Loaded in {elapsed:.2f}s")
...
def checkout():
    ...
    try:
        cart_items, cart_total = get_cart_with_details_fast(session['user_id'])
...
def cart_ultrafast():
    ...
    try:
        cart_items, total_amount = get_cart_with_details_fast(session['user_id'])
...
if __name__ == '__main__':
    ...
    if not is_render:
        ...
        print(f"✅ CART FIX: Cart display issue resolved")
        ...
        print("✅ CART FIX: Cart display issue resolved")
