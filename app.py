...
# ============================================
# ✅ ORDER HISTORY ROUTE (COMPLETELY FIXED WITH CUSTOMER ORDER NUMBERING)
# ============================================
...
def order_history():
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    ...
                    FROM orders o
                    WHERE o.user_id = %s 
                    ORDER BY o.order_date DESC  # ✅ Latest order FIRST
                """, (session['user_id'],))
                ...
                for index, order in enumerate(orders_data):
                    # ✅ FIXED: Customer order number = index + 1
                    # Latest order (index=0) → Order #1
                    # Second order (index=1) → Order #2
                    customer_order_no = index + 1
                    
                    # ✅ FIX: Convert dates to IST
                    ...
                    orders_list.append({
                        'order_id': order['order_id'],      # Internal database ID
                        'order_no': customer_order_no,      # ✅ Customer-facing order number (1, 2, 3...)
        ...
        if orders_list and len(orders_list) > 0:
            ...
            # Debug: Show order numbers
            for order in orders_list:
                print(f"🔍 [ORDER_HISTORY] Internal Order ID: {order['order_id']}, Customer Order No: {order['order_no']}")
...
# ============================================
# ✅ ORDER DETAILS ROUTE (COMPLETELY FIXED WITH CUSTOMER ORDER NUMBERING)
# ============================================
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                # ✅ NEW: Get user's total orders and this order's position
                cur.execute("""
                ...
                # ✅ Get position of this order (latest = 1, oldest = N)
                cur.execute("""
                    SELECT COUNT(*) as position
                    FROM orders 
                    WHERE user_id = %s 
                    AND order_date >= %s
                """, (session['user_id'], order['order_date']))
                
                position_result = cur.fetchone()
                customer_order_no = position_result['position'] if position_result else 0
                
                # ✅ CRITICAL FIX: Convert ALL dates to IST timezone
                # Convert order_date to IST and format
                if order.get('order_date'):
                    order['order_date_formatted'] = format_ist_datetime(
                        order['order_date'], 
                        "%d %b %Y, %I:%M %p"  # Example: "18 Jan 2026, 10:54 PM"
                    )
                    print(f"✅ [ORDER_DETAILS] Original order_date: {order['order_date']}")
                    print(f"✅ [ORDER_DETAILS] Formatted order_date: {order['order_date_formatted']}")
                else:
                    order['order_date_formatted'] = 'Date not available'
                
                # Convert delivery_date if exists
                if order.get('delivery_date'):
                    order['delivery_date_formatted'] = format_ist_datetime(
                        order['delivery_date'],
                        "%d %b %Y, %I:%M %p"
                    )
                    print(f"✅ [ORDER_DETAILS] Formatted delivery_date: {order.get('delivery_date_formatted')}")
                ...
                try:
                    ...
                    if payment_result:
                        ...
                        # ✅ FIX: Convert payment_date to IST
                        if order.get('payment_date'):
                            order['payment_date_formatted'] = format_ist_datetime(
                                order['payment_date'],
                                "%d %b %Y, %I:%M %p"
                            )
                            print(f"✅ [ORDER_DETAILS] Formatted payment_date: {order['payment_date_formatted']}")
                except Exception as e:
                ...
                print(f"✅ [ORDER_DETAILS] Internal Order ID: {order_id}")
                print(f"✅ [ORDER_DETAILS] Customer Order No: {customer_order_no}")
                print(f"✅ [ORDER_DETAILS] Items count: {len(items_list)}")
                print(f"✅ [ORDER_DETAILS] Order status: {order['status']}")
                print(f"✅ [ORDER_DETAILS] Order date formatted: {order.get('order_date_formatted')}")
                
                # ✅ NEW: Add customer order number to order dict
