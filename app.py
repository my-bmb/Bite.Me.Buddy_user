# app.py - COMPLETE UPDATED VERSION WITH DYNAMIC CATEGORY SERVICES FOR BOTH SERVICES & MENU
import os
...
def service_details(service_id):
    ...
    try:
        ...
        # Get all services from same category (for category services section)
        category_services = []
        if category:
            category_services_list = supabase_execute(
                'services',
                'select',
                conditions={'category_id': category['id'], 'status': 'active'}
            )
            category_services = sorted(category_services_list, key=lambda x: x.get('position', 0)) if category_services_list else []
            print(f"📊 [SERVICE-DETAIL] Found {len(category_services)} services in category {category.get('name')}")
        ...
        # Get recommended services (other categories) with ALL columns
        recommended_services = []
        if category:
            # Get all services from other categories
            all_services = supabase_execute(
                'services',
                'select',
                conditions={'status': 'active'}
            )
            # Filter out current category services and current service
            recommended = [item for item in all_services if item.get('category_id') != category['id'] and item['id'] != service_id]
            recommended_services = sorted(recommended, key=lambda x: x.get('position', 0))[:6]
            print(f"📊 [SERVICE-DETAIL] Found {len(recommended_services)} recommended services from other categories")
        ...
        # ✅ Get all categories except current category (for Browse By Category)
        other_categories = [cat for cat in all_categories if cat['id'] != category['id']] if category else all_categories
        
        # ✅ Get services for first other category (for initial Category Services display)
        first_category_services = []
        if other_categories and len(other_categories) > 0:
            first_category_id = other_categories[0]['id']
            first_category_services = supabase_execute(
                'services',
                'select',
                conditions={'category_id': first_category_id, 'status': 'active'}
            )
            # Add default photos
            for service in first_category_services:
                if not service.get('photo'):
                    service['photo'] = "https://res.cloudinary.com/demo/image/upload/v1633427556/sample_service.jpg"
        ...
        return render_template('service_details.html', 
                             ...
                             first_category_services=first_category_services,
                             all_categories=all_categories,
                             all_collections=all_collections,
                             active_tab='services')
...
# ============================================
# ✅ API ENDPOINTS FOR DYNAMIC CATEGORY SERVICES
# ============================================
...
def menu_item_details(item_id):
    ...
    try:
        ...
        # Get all items from same category (for category items section)
        category_items = []
        if category:
            category_items_list = supabase_execute(
                'menu_items',
                'select',
                conditions={'category_id': category['id'], 'status': 'active'}
            )
            category_items = sorted(category_items_list, key=lambda x: x.get('position', 0)) if category_items_list else []
            print(f"📊 [MENU-ITEM] Found {len(category_items)} items in category {category.get('name')}")
        ...
        # Get recommended items (other categories) with ALL columns
        recommended_items = []
        if category:
            # Get all items from other categories
            all_items = supabase_execute(
                'menu_items',
                'select',
                conditions={'status': 'active'}
            )
            # Filter out current category items and current item
            recommended = [item for item in all_items if item.get('category_id') != category['id'] and item['id'] != item_id]
            recommended_items = sorted(recommended, key=lambda x: x.get('position', 0))[:6]
            print(f"📊 [MENU-ITEM] Found {len(recommended_items)} recommended items from other categories")
        ...
        # ✅ Get items for first other category (for initial Category Items display)
        first_category_items = []
        if other_categories and len(other_categories) > 0:
            first_category_id = other_categories[0]['id']
            first_category_items = supabase_execute(
                'menu_items',
                'select',
                conditions={'category_id': first_category_id, 'status': 'active'}
            )
            # Add default photos
            for item in first_category_items:
                if not item.get('photo'):
                    item['photo'] = "https://res.cloudinary.com/demo/image/upload/v1633427556/sample_food.jpg"
        ...
        return render_template('menu_item_details.html', 
                             ...
                             other_categories=other_categories,
                             first_category_items=first_category_items,
                             all_categories=all_categories,
                             all_collections=all_collections,
                             active_tab='menu')
