# app.py - COMPLETE UPDATED VERSION WITH SUPABASE DIRECT FETCH FOR SERVICES & MENU
import os
...
# ============================================
# ✅ SERVICES SYSTEM - UPDATED (Direct Supabase fetch)
# ============================================
...
def services():
    ...
    try:
        ...
        # For each category, get service count
        for category in categories:
            service_count = supabase_execute(
                'services',
                'select',
                conditions={'category_id': category['id'], 'status': 'active'}
            )
            category['service_count'] = len(service_count) if service_count else 0
...
def service_category_items(category_id):
    """Display all services for a specific category"""
    ...
    try:
        ...
        if services_list:
            ...
            for idx, service in enumerate(services_list):
                print(f"  Service {idx+1}: {service.get('name')} - Price: ₹{service.get('final_price', 0)}")
...
def service_details(service_id):
    """Display detailed view of a single service with recommendations"""
    ...
    try:
        ...
        print(f"📊 [SERVICE-DETAIL] Service: {service.get('name')} - Price: ₹{service.get('final_price', 0)}")
        ...
        # Get recommended services (same category, different services)
        ...
        # Get all categories for horizontal scroll
        all_categories = supabase_execute(
            'service_categories',
            'select',
            conditions={'status': 'active'}
...
# ============================================
# ✅ MENU SYSTEM - UPDATED (Direct Supabase fetch)
# ============================================
...
def menu():
    ...
    try:
        ...
        # For each category, get item count
        for category in categories:
            item_count = supabase_execute(
                'menu_items',
                'select',
                conditions={'category_id': category['id'], 'status': 'active'}
            )
            category['item_count'] = len(item_count) if item_count else 0
...
def menu_category_items(category_id):
    """Display all menu items for a specific category"""
    ...
    try:
        ...
        if menu_items:
            ...
            for idx, item in enumerate(menu_items):
                print(f"  Item {idx+1}: {item.get('name')} - Price: ₹{item.get('final_price', 0)}")
...
def menu_item_details(item_id):
    """Display detailed view of a single menu item with recommendations"""
    ...
    try:
        ...
        print(f"📊 [MENU-ITEM] Item: {menu_item.get('name')} - Price: ₹{menu_item.get('final_price', 0)}")
        ...
        # Get recommended items (same category, different items)
        recommended_items = []
        if category:
            recommended = supabase_execute(
                'menu_items',
