# app.py - COMPLETE UPDATED VERSION WITH ORDER ITEMS NORMALIZATION FIX & TRENDING FIX
import os
...
# ============================================
# ✅ MAIN PAGES ROUTES - UPDATED DASHBOARD WITH COLLECTIONS
# ============================================
...
# ============================================
# ✅ TRENDING ITEMS FUNCTION - UPDATED WITH JSONB & STRING HANDLING
# ============================================
...
def get_trending_items(limit=10):
    ...
    Handles BOTH JSONB and String formats in items column
    """
    try:
        from datetime import datetime, timedelta
        from dateutil import parser
        
        print(f"\n🔍 [TRENDING] Starting trending calculation...")
        ...
        if not orders:
            print("⚠️ [TRENDING] No orders found in database")
            return []
        
        print(f"📊 [TRENDING] Total orders found: {len(orders)}")
        
        # Filter orders from last 30 days with proper date comparison
        recent_orders = []
        ...
        for order in orders:
            ...
            try:
                # Parse order_date safely (handles string, datetime, timestamp)
                ...
                if order_date > thirty_days_ago:
                    ...
                    print(f"✅ [TRENDING] Order {order.get('order_id')} from {order_date.strftime('%Y-%m-%d')} is recent")
                else:
                    print(f"⏭️ [TRENDING] Order {order.get('order_id')} from {order_date.strftime('%Y-%m-%d')} is older than 30 days")
                    
            except Exception as e:
                print(f"⚠️ [TRENDING] Date parse error for order {order.get('order_id')}: {e}")
                continue
        
        print(f"📊 [TRENDING] Found {len(recent_orders)} orders in last 30 days")
        
        if not recent_orders:
            print("⚠️ [TRENDING] No orders in last 30 days")
            return []
        ...
        for order in recent_orders:
            ...
            if not items_raw:
                print(f"⚠️ [TRENDING] Order {order.get('order_id')} has no items")
                continue
            
            try:
                # ✅ CRITICAL FIX: Handle both JSONB and String formats
                if isinstance(items_raw, str):
                    # It's a string, parse it
                    items = json.loads(items_raw)
                    print(f"  📝 Parsed string JSON for order {order.get('order_id')}")
                elif isinstance(items_raw, (list, dict)):
                    # It's already JSONB/object
                    items = items_raw if isinstance(items_raw, list) else [items_raw]
                    print(f"  ✅ Direct JSONB for order {order.get('order_id')}")
                else:
                    print(f"  ⚠️ Unknown items format: {type(items_raw)} for order {order.get('order_id')}")
                    continue
                
                # Handle single item as dict (if needed)
                ...
                for item in items:
                    if not isinstance(item, dict):
                        print(f"  ⚠️ Item is not dict: {type(item)}")
                        continue
                    
                    item_id = item.get('item_id') or item.get('id')
                    item_type = item.get('item_type') or item.get('type')
                    item_name = item.get('item_name') or item.get('name') or 'Unknown'
                    item_photo = item.get('item_photo') or item.get('photo') or ''
                    item_price = item.get('price') or item.get('final_price') or 0
                    quantity = item.get('quantity', 1)
                    
                    # Skip invalid items
                    if not item_id or not item_type:
                        print(f"  ⚠️ Skipping item with missing id/type: {item}")
                        continue
                    ...
                    if key not in item_count:
                        ...
                        if not item_photo or not item_photo.startswith('http'):
                            if item_type == 'service':
                                item_photo = "https://res.cloudinary.com/demo/image/upload/v1633427556/sample_service.jpg"
                            else:
                                item_photo = "https://res.cloudinary.com/demo/image/upload/v1633427556/sample_goods.jpg"
                    ...
                    # Increment count (using quantity)
                    item_count[key] += int(quantity) if quantity else 1
                    
            except json.JSONDecodeError as e:
                print(f"⚠️ [TRENDING] JSON decode error for order {order.get('order_id')}: {e}")
                print(f"   Raw items: {str(items_raw)[:200]}")
                continue
            except Exception as e:
                print(f"⚠️ [TRENDING] Item processing error for order {order.get('order_id')}: {e}")
                continue
        
        if not item_count:
            print("⚠️ [TRENDING] No items found in recent orders")
            return []
        ...
        for key, count in trending[:limit]:
            if key in item_details:
                ...
                print(f"  🔥 {details['name']} ({details['type']}) - Ordered {count} times")
        
        print(f"✅ [TRENDING] Successfully found {len(trending_items)} trending items")
        return trending_items
        
    except Exception as e:
        print(f"❌ [TRENDING] Fatal error: {e}")
        import traceback
        traceback.print_exc()
        return []
...
# ============================================
# ✅ CHECKOUT & ORDERS ROUTES - SUPABASE (FIXED WITH PROPER ITEMS FORMAT)
# ============================================
...
def checkout():
    if request.method == 'POST':
        ...
        try:
            ...
            total_amount = 0
            items_list = []  # ✅ This will be a LIST of items
            ...
            for item in cart_items:
                ...
                if details:
                    ...
                    # ✅ ALWAYS store as dict with standardized keys
                    items_list.append({
...
# ============================================
# ✅ ORDER HISTORY ROUTE - SUPABASE (FIXED WITH NORMALIZATION)
# ============================================
...
def order_history():
    try:
        ...
        for index, order in enumerate(orders_data):
            ...
            # ✅ CRITICAL FIX: Use normalize_items helper for consistency
        ...
        for order in orders_list:
            print(f"  - Order #{order['order_no']}: {order['order_date_formatted']} ({len(order['items'])} items)")
...
# ============================================
# ✅ ORDER DETAILS ROUTE - SUPABASE (COMPLETELY FIXED WITH NORMALIZATION)
# ============================================

@app.route('/order/<int:order_id>')
@login_required
def order_details(order_id):
    """View detailed order information with IST timezone conversion - FULLY FIXED for service orders"""
    try:
        print(f"\n🔍 [ORDER_DETAILS] STARTING for order_id: {order_id}")
        print(f"🔍 [ORDER_DETAILS] User ID from session: {session.get('user_id')}")
        ...
        print(f"🔍 [ORDER_DETAILS] Orders found: {len(orders) if orders else 0}")
        ...
        # ✅ CRITICAL FIX: Use the normalize_items helper
        print(f"\n🔍 [ORDER_DETAILS] Normalizing items for order {order_id}")
        print(f"  Raw items type: {type(order.get('items'))}")
        print(f"  Raw items preview: {str(order.get('items'))[:200]}...")
        
        items_list = normalize_order_items(order.get('items'))
        
        print(f"✅ [ORDER_DETAILS] Normalized {len(items_list)} items")
        for idx, item in enumerate(items_list):
            print(f"   Item {idx+1}: {item['name']} (Type: {item['type']}, Qty: {item['quantity']}, Price: ₹{item['price']})")
        
        # Add customer order number to order dict
        order['order_no'] = customer_order_no
        
        print(f"✅ [ORDER_DETAILS] Final - Order ID: {order_id}, Customer Order No: {order['order_no']}, Items: {len(items_list)}")
...
# ============================================
# ✅ SERVICE & GOODS DETAILS ROUTES - SUPABASE
# ============================================
...
def debug_data():
    ...
    try:
        ...
        print(f"📊 [DEBUG] goods_collections: {debug_info['goods_collections']['count']} records")
        
        # Check goods_categories
        ...
        print(f"📊 [DEBUG] goods_categories: {debug_info['goods_categories']['count']} records")
        ...
        print(f"📊 [DEBUG] goods_items: {debug_info['goods_items']['count']} records")
...
# ============================================
# ✅ DATA MIGRATION: FIX ALL EXISTING ORDERS (Run once)
# ============================================
...
def migrate_fix_all_orders():
    ...
    # Only allow admin or specific user for security
    # Change this to your admin user ID or remove for production after migration
    if session.get('user_id') != 1:  # Change to your admin user ID
    ...
    try:
        ...
        for order in all_orders:
            ...
            if normalized_items:
                ...
                print(f"✅ Order #{order_id}: Fixed ({len(normalized_items)} items)")
            else:
                # No items or error
                if original_items:
                    error_count += 1
        ...
        <html>
        <head>
            <title>Order Migration Report</title>
            <style>
                body {{ font-family: Arial, sans-serif; margin: 20px; background: #0D0D0D; color: #F4F1E1; }}
                h1 {{ color: #D4AF37; }}
        ...
        <body>
            ...
            <p><strong>Total Orders Processed:</strong> {len(all_orders)}</p>
            <p><strong>Successfully Fixed:</strong> <span class="success">{fixed_count}</span></p>
            <p><strong>Errors:</strong> <span class="error">{error_count}</span></p>
            <hr>
            <h3>Details:</h3>
            <ul>
                {' '.join([f'<li>{r}</li>' for r in results[:50]])}
            </ul>
            <hr>
