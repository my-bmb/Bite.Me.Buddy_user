...
# ✅ FIXED: All chat routes now use session-based user_id (removed current_user dependency)
# ✅ FIXED: register-chat and login-chat routes rewritten to use session-based auth
...
# ✅ Flask-Login Setup (kept but not actively used for auth; we use session-based)
...
# ============================================
# ✅ PERSISTENT SESSION - Make all sessions permanent
# ============================================
...
# ============================================
# ✅ UUID FIX FUNCTIONS (CRITICAL - simplified for integer IDs)
# ============================================

def get_user_uuid_by_phone(phone):
    """Fetch the correct user ID (int or UUID) from the users table using phone number"""
    try:
        ...
        print(f"❌ UUID fetch error: {e}")
        return None

def ensure_uuid_user():
    """
    Return user_id from session (IDs are integers, no UUID conversion needed).
    If user_id is missing but phone exists, try to fetch correct ID.
    Returns the user_id (int) or None if it cannot be fixed.
    """
    ...
    # No need to validate UUID format - accept any ID (int, str, UUID)
...
def get_session_user():
    """Return a dict with user info from session, or None if not logged in."""
    user_id = session.get('user_id')
    if not user_id:
        return None
    try:
...
def get_current_user_from_session():
    """Return a simple object with user attributes (compatible with templates)."""
    user_data = get_session_user()
    if not user_data:
        return None
    # Return a simple object (or dict) with attributes
    class UserObj:
        def __init__(self, data):
            self.id = data['id']
            self.full_name = data.get('full_name', data.get('username', 'User'))
            self.username = data.get('username', self.full_name)
            self.email = data.get('email', '')
            self.profile_pic = data.get('profile_pic', DEFAULT_AVATAR_URL)
        ...
        try:
            ...
            if new_user and len(new_user) > 0:
                ...
                if latitude is not None and longitude is not None:
                    ...
                    print(f"✅ [REGISTER] Coordinates stored in session")
                ...
                print(f"  - Location details saved: {'YES' if 'location_details' in user_data else 'NO'}")
        ...
        try:
            ...
            if users and len(users) > 0:
                ...
                if check_password_hash(user['password'], password):
                    ...
                    # Set session data
                    session['user_id'] = user['id']  # This is integer ID
                    ...
                    if parsed_location['is_auto_detected']:
                        ...
                        # Load coordinates from database if available
                        session['latitude'] = user['latitude']
                        session['longitude'] = user['longitude']
                        print(f"✅ [LOGIN] Loaded coordinates from database: {user['latitude']}, {user['longitude']}")
...
# ============================================
# ✅ PROFILE CHAT ROUTE (USES profile_chat.html) - SEPARATE TEMPLATE
# ============================================

@app.route('/profile-chat')
@login_required
def profile_chat():
    # Get user from session instead of current_user
    user_id = session.get('user_id')
    if not user_id:
    ...
    try:
        ...
        if session.get('user_lat') and session.get('user_lng'):
            ...
            if user_lat is None or user_lng is None:
                # Try to parse from location_wkt or location
        ...
        # ✅ USING SEPARATE TEMPLATE FOR CHAT PROFILE
...
def referral():
    ...
    try:
        # ✅ CRITICAL: Ensure we have the correct user_id
        user_id = ensure_uuid_user()
        if not user_id:
...
def get_referral_stats():
    ...
    try:
        # Get referred users
        referred_users = supabase_execute('users', 'select', 
                                          conditions={'referral_mobile': session.get('phone')},
                                          use_admin=True)
...
# ============================================
# Note: Flask-Login is kept for compatibility but not used for actual auth.
# All auth is session-based.
...
# ---- register-chat and login-chat are kept for compatibility but rewritten to use session-based auth ----
@app.route('/register-chat', methods=['GET', 'POST'])
def register_chat():
    # If user is already logged in via session, redirect to users_chat
    ...
    if request.method == 'POST':
        ...
        existing = supabase_execute_safe(lambda: supabase.table('users').select('*').eq('full_name', username).execute())
        if existing:
            flash('Username already exists', 'danger')
            return redirect(url_for('register_chat'))
        existing_email = supabase_execute_safe(lambda: supabase.table('users').select('*').eq('email', email).execute())
        if existing_email:
        ...
        try:
            ...
            if r.data:
                ...
                return redirect(url_for('login'))  # Use main login
        except Exception as e:
...
def login_chat():
    ...
    if request.method == 'POST':
        username = request.form.get('username', '').strip()
        password = request.form.get('password', '').strip()
        user_data = supabase_execute_safe(lambda: supabase.table('users').select('*').eq('full_name', username).execute())
        if user_data and check_password_hash(user_data[0]['password'], password):
            ...
            session['email'] = user['email']
            session['profile_pic'] = user.get('profile_pic', DEFAULT_AVATAR_URL)
            session.permanent = True
            # Update online status
            try:
                supabase.table('users').update({'is_online': True, 'last_seen': None}).eq('id', user['id']).execute()
...
@login_required
def users_chat():
    # ✅ Session se user_id lo (Flask-Login ke current_user ki jagah)
    user_id = session.get('user_id')
    if not user_id:
        flash('Please login first', 'error')
        return redirect(url_for('login'))
...
def chat(user_id):
    ...
    # Build current user object for template
    user_obj = {
        'id': current_user_id,
    ...
    return render_template('chat.html', other_user=other_user, messages=messages, current_user=user_obj)
...
def load_more_messages():
    ...
    if not current_user_id:
        return jsonify([])
    messages = get_messages_between(current_user_id, other_user_id, limit=20, offset=offset)
    if messages:
        message_ids = [msg['id'] for msg in messages]
        reactions_by_msg = get_reactions_for_messages(message_ids)
        for msg in messages:
...
def edit_message_route():
    ...
    if not current_user_id:
        return jsonify({'error': 'Not authenticated'}), 401
    if edit_message(message_id, current_user_id, new_content):
        socketio.emit('message_edited', {'message_id': message_id, 'new_content': new_content}, room=receiver_id)
        socketio.emit('message_edited', {'message_id': message_id, 'new_content': new_content}, room=str(current_user_id))
        return jsonify({'success': True})
...
def react_to_message():
    ...
    if not current_user_id:
        return jsonify({'error': 'Not authenticated'}), 401
    existing = supabase.table('message_reactions').select('*').eq('message_id', message_id).eq('user_id', current_user_id).eq('reaction', reaction).execute()
    if existing.data:
        remove_reaction(message_id, current_user_id, reaction)
    else:
        add_reaction(message_id, current_user_id, reaction)
    ...
    socketio.emit('reaction_updated', {'message_id': message_id, 'reactions': new_reactions}, room=str(current_user_id))
...
def search_messages():
    ...
    other_user_id = request.args.get('other_user_id')
    current_user_id = session.get('user_id')
    if not current_user_id or not q or not other_user_id:
        return jsonify([])
    try:
        r = supabase.table('messages').select('*')\
            .or_(f"and(sender_id.eq.{current_user_id},receiver_id.eq.{other_user_id}),and(sender_id.eq.{other_user_id},receiver_id.eq.{current_user_id})")\
            .ilike('content', f'%{q}%')\
            .order('created_at', desc=True)\
            .limit(30)\
            .execute()
...
def upload_file():
    ...
    message = save_message(current_user_id, receiver_id, msg_type, public_url)
    if message:
        msg_dict = {
            'id': message['id'],
        ...
        socketio.emit('new_message', msg_dict, room=str(current_user_id))
        unread_count = get_unread_counts(receiver_id).get(current_user_id, 0)
        socketio.emit('unread_update', {'sender_id': current_user_id, 'count': unread_count}, room=receiver_id)
...
def upload_audio():
    ...
    try:
        ...
        return {'error': 'Upload failed'}, 500
    message = save_message(current_user_id, receiver_id, 'audio', public_url)
    if message:
        msg_dict = {
            'id': message['id'],
            'sender_id': message['sender_id'],
...
def delete_message_route(message_id):
    ...
    try:
        ...
        msg = msg.data[0]
        if msg['sender_id'] != current_user_id and delete_for == 'everyone':
        ...
        if delete_for == 'everyone':
            ...
            socketio.emit('message_deleted', {'message_id': message_id, 'for_everyone': False}, room=current_user_id)
        return jsonify({'success': True})
    except Exception as e:
...
# ============================================
# ✅ EDIT PROFILE ROUTE (CHAT) - USES edit_profile.html
# ============================================
...
def edit_profile():
    ...
    if request.method == 'POST':
        try:
            ...
            if 'profile_pic' in request.files:
                ...
                if file and file.filename:
                    ...
                    if ext in ['png', 'jpg', 'jpeg', 'gif', 'webp']:
                        ...
                        file_path = f"profile_photos/{current_user_id}/{filename}"
                        file_bytes = file.read()
                        supabase.storage.from_('chat-files').upload(file_path, file_bytes)
            ...
            existing_user = supabase.table('users').select('photos', 'profile_pic').eq('id', current_user_id).execute()
...
def view_user_profile(user_id):
    ...
    try:
        ...
        # Build current_user object for template
        current_user_obj = {
...
def live_chat():
    try:
        ...
        # Build current_user object for template (using session data)
        own_result = supabase.table('users').select('*').eq('id', user_id).execute()
        if not own_result.data:
            flash('User not found', 'danger')
            return redirect(url_for('logout'))
        own_data = own_result.data[0]
        user_obj = {
            'id': own_data['id'],
            'full_name': own_data['full_name'],
            'username': own_data['full_name'],
            'email': own_data['email'],
            'is_online': own_data.get('is_online', False),
            'last_seen': own_data.get('last_seen'),
            'profile_pic': own_data.get('profile_pic'),
            'age': own_data.get('age'),
            'bio': own_data.get('bio'),
            'interests': own_data.get('interests')
        }
...
def get_live_chat_messages():
    try:
        ...
        current_user_name = session.get('full_name', 'User')
        current_user_id = session.get('user_id')
        return jsonify({
            'success': True,
            'messages': formatted_messages,
...
def handle_edit_message(data):
    ...
    if not user_id:
        return
    if edit_message(data['message_id'], user_id, data['new_content']):
        emit('message_edited', {'message_id': data['message_id'], 'new_content': data['new_content']}, room=data['receiver_id'])
        emit('message_edited', {'message_id': data['message_id'], 'new_content': data['new_content']}, room=str(user_id))
...
def handle_typing(data):
    ...
    if not user_id:
        return
    emit('user_typing', {'user_id': user_id, 'is_typing': data['is_typing']}, room=data['receiver_id'])
...
def handle_join_live_chat():
    ...
    if user_id:
        join_room('live_chat')
        logger.info(f"User {user_name} joined live chat")
        notification = {
            'type': 'join',
            'user_id': user_id,
            'user_name': user_name,
            'message': f"✨ {user_name} joined the live chat",
...
def handle_leave_live_chat():
    ...
    if user_id:
        ...
        logger.info(f"User {user_name} left live chat")
        notification = {
            'type': 'leave',
            'user_id': user_id,
            'user_name': user_name,
            'message': f"🚪 {user_name} left the live chat",
            'timestamp': get_utc_time(),
            'formatted_time': format_ist_time(get_utc_time())
        }
        emit('live_user_join_leave', notification, room='live_chat')
...
def handle_join_group_call_room(data):
    ...
    if user_id:
        ...
        if call_id:
            ...
            join_room(room_name)
            logger.info(f"User {user_name} joined group call room {call_id}")
...
def handle_ice_candidate(data):
    ...
    if target_id and candidate:
        ...
        logger.info(f"❄️ ICE candidate forwarded from {user_id} to {target_id}")
...
if __name__ == '__main__':
    ...
    if not is_render:
        ...
        print(f"✅ FIXED: All chat routes now use session-based user_id (removed current_user dependency)")
        print(f"✅ FIXED: register-chat and login-chat rewritten to use session-based auth")
        ...
        print("✅ FIXED: All chat routes now use session-based user_id (removed current_user dependency)")
        print("✅ FIXED: register-chat and login-chat rewritten to use session-based auth")
