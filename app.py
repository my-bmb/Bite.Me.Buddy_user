...
def order_details(order_id):
    ...
    try:
        ...
        # ✅ CRITICAL FIX: Parse items JSON with proper error handling
        items_list = []
        if order.get('items'):
            try:
                json_items = json.loads(order['items'])
                ...
                if isinstance(json_items, list):
                    ...
                    print(f"⚠️ [ORDER_DETAILS] Items is not a list: {type(json_items)}")
                    items_list = []
                    
            except json.JSONDecodeError as e:
                print(f"❌ [ORDER_DETAILS] JSON decode error for order {order_id}: {e}")
                print(f"   Items string: {order['items'][:200]}...")
                items_list = []
            except Exception as e:
