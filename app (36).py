# app.py - COMPLETE UPDATED VERSION WITH SUPABASE INTEGRATION (FIXED FOR v2.0+)
import os
...
def to_ist(datetime_obj):
    ...
    Handles: None, string, naive datetime, UTC datetime, IST datetime
    """
    ...
    # ✅ Ab datetime object ke saath kaam karo
    try:
...
def format_ist_datetime(datetime_obj, format_str="%d %b %Y, %I:%M %p"):
    ...
    # ✅ FIXED: Agar string hai to wapas string bhejo
    if isinstance(datetime_obj, str):
        return datetime_obj
...
def init_database():
    ...
    try:
        # ✅ FIXED: Test connection with new syntax
...
def order_history():
    try:
        ...
        for index, order in enumerate(orders_data):
            ...
            # ✅ FIXED: Format dates to IST safely
            if order.get('order_date'):
                try:
                    ...
                    print(f"⚠️ [ORDER_HISTORY] Date format error: {e}")
            ...
            if order.get('delivery_date'):
                try:
                    ...
                    print(f"⚠️ [ORDER_HISTORY] Delivery date format error: {e}")
...
def order_details(order_id):
    ...
    try:
        ...
        # ✅ FIXED: Convert dates to IST safely
        if order.get('order_date'):
            try:
                order['order_date_formatted'] = format_ist_datetime(
                    order['order_date'], 
                ...
                print(f"⚠️ [ORDER_DETAILS] Date format error: {e}")
                order['order_date_formatted'] = str(order['order_date'])
        else:
            order['order_date_formatted'] = 'Date not available'
        ...
        if order.get('delivery_date'):
            try:
                ...
                print(f"⚠️ [ORDER_DETAILS] Delivery date format error: {e}")
                order['delivery_date_formatted'] = str(order['delivery_date'])
            ...
            if payments:
                ...
                if order.get('payment_date'):
                    try:
                        ...
                        print(f"⚠️ [ORDER_DETAILS] Payment date format error: {e}")
