...
# ============================================
# ✅ SOCKETIO EVENTS (UNIFIED USERS TABLE)
# ============================================
...
def handle_connect():
    ...
    try:
        supabase.table('users').update({'is_online': True, 'last_seen': None}).eq('id', current_user.id).execute()
...
def handle_disconnect():
    ...
    if current_user.is_authenticated:
        ...
        try:
            supabase.table('users').update({'is_online': False, 'last_seen': get_utc_time()}).eq('id', current_user.id).execute()
