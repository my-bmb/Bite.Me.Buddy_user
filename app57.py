# app.py - COMPLETE UPDATED VERSION WITH FIXED CUSTOMER ORDER NUMBERING
import os
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                # ✅ FIXED: Get ALL orders for this user sorted by date
                cur.execute("""
                ...
                # ✅ FIXED: Find position of this order in user's order history
                customer_order_no = None
                for index, user_order in enumerate(all_orders):
                    if user_order['order_id'] == order_id:
                        # Customer order number = total_orders - index
                        customer_order_no = len(all_orders) - index
                        break
                
                if customer_order_no is None:
                    customer_order_no = 1  # Fallback
                
                print(f"✅ [ORDER_DETAILS] User {session['user_id']} has {len(all_orders)} total orders")
                print(f"✅ [ORDER_DETAILS] Order {order_id} is customer order #{customer_order_no}")
