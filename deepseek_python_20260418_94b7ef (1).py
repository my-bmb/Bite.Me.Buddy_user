...
# ✅ FIXED: UUID type mismatch – added ensure_uuid_user() to convert session user_id to UUID
...
# ============================================
# ✅ UUID FIX FUNCTIONS (CRITICAL)
# ============================================

def get_user_uuid_by_phone(phone):
    """Fetch the correct UUID from the users table using phone number"""
    try:
...
def ensure_uuid_user():
    """
    Ensure that session['user_id'] is a valid UUID.
    If it's not, try to fetch the correct UUID using session['phone'].
    Returns the valid UUID or None if it cannot be fixed.
    """
    ...
    try:
        ...
        print(f"✅ Session user_id is already a valid UUID: {user_id}")
        return user_id
    except ValueError:
        print(f"⚠️ Invalid session user_id (not a UUID): {user_id}")
    
    # Try to fix by looking up the phone number
    phone = session.get("phone")
    if not phone:
        print("❌ No phone in session, cannot fix UUID")
        return None
    
    correct_uuid = get_user_uuid_by_phone(phone)
    if correct_uuid:
        session["user_id"] = correct_uuid
        print(f"✅ Fixed session user_id to UUID: {correct_uuid}")
        return correct_uuid
    else:
        print("❌ Could not fetch correct UUID from database")
        return None
...
# ============================================
# ✅ REFERRAL REWARD FUNCTION (FIXED: changed 'mobile' to 'phone')
# ============================================
...
def process_referral_reward(user_id, amount_spent):
    ...
    Args:
        user_id: ID of the user who spent money (should be UUID)
...
# ============================================
# ✅ AUTHENTICATION ROUTES - WITH REFERRAL SUPPORT (FIXED: changed 'mobile' to 'phone')
# ============================================
        ...
        try:
            ...
            if users and len(users) > 0:
                ...
                if check_password_hash(user['password'], password):
                    ...
                    # Set session data
                    session['user_id'] = user['id']  # This is UUID
...
def dashboard():
    ...
    try:
        # Ensure we have the correct UUID
...
# ============================================
# ✅ PROFILE ROUTE - CLEAN VERSION (NO REFERRAL/WALLET DETAILS)
# ============================================
...
def referral():
    ...
    try:
        # ✅ CRITICAL: Ensure we have the correct UUID
        user_id = ensure_uuid_user()
        if not user_id:
...
if __name__ == '__main__':
    ...
    if not is_render:
        ...
        print(f"✅ UUID FIX: ensure_uuid_user() corrects session user_id")
        ...
        print("✅ UUID FIX: ensure_uuid_user() corrects session user_id")
