    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if new_user and len(new_user) > 0:
                ...
                # ✅ FIX: Create Flask-Login user object and log the user in
                user_obj = User(
                    id=user_id,
                    username=full_name,
                    email=email,
    ...
    if request.method == 'POST':
        ...
        if user_data and check_password_hash(user_data[0]['password'], password):
            user = User(
                ...
                user_data[0]['email'], 
                user_data[0]['is_online']
            )
            login_user(user, remember=True)
            session.permanent = True
            session['user_id'] = user.id
            session['full_name'] = user_data[0]['full_name']
            session['username'] = user_data[0]['full_name']
...
def chat(user_id):
    try:
        # ✅ Get current user id from session (instead of current_user.id)
        current_user_id = session.get('user_id')
        if not current_user_id:
            flash('Please login first', 'error')
            return redirect(url_for('login'))
        
        # ✅ Fetch current user data for template
        current_user_data = supabase.table('users').select('*').eq('id', current_user_id).execute()
        if not current_user_data.data:
            flash('User data not found', 'danger')
            return redirect(url_for('logout'))
        current_user_obj = User(
            id=current_user_data.data[0]['id'],
            username=current_user_data.data[0]['full_name'],
            email=current_user_data.data[0]['email'],
            is_online=current_user_data.data[0].get('is_online', False),
            last_seen=current_user_data.data[0].get('last_seen'),
            profile_pic=current_user_data.data[0].get('profile_pic'),
            age=current_user_data.data[0].get('age'),
            bio=current_user_data.data[0].get('bio'),
            interests=current_user_data.data[0].get('interests')
        )
    ...
    # ✅ Use current_user_obj.id instead of current_user.id
    messages = get_messages_between(current_user_obj.id, user_id, limit=20, offset=0)
    ...
    marked = mark_messages_as_read(current_user_obj.id, user_id)
    if marked:
        socketio.emit('messages_read', {'reader_id': current_user_obj.id, 'sender_id': user_id}, room=user_id)
        unread_after = get_unread_counts(current_user_obj.id).get(user_id, 0)
        socketio.emit('unread_update', {'sender_id': user_id, 'count': unread_after}, room=str(current_user_obj.id))
    
    return render_template('chat.html', other_user=other_user, messages=messages, current_user=current_user_obj)
...
def audio_call(user_id):
    try:
        ...
        user_result = supabase.table('users').select('full_name').eq('id', current_user_id).execute()
        current_user_name = user_result.data[0]['full_name'] if user_result.data else 'User'
...
def react_to_live_message():
    try:
        ...
        user_name = user_result.data[0]['full_name'] if user_result.data else 'User'
...
def group_video_call():
    try:
        ...
        user_result = supabase.table('users').select('id, full_name').eq('id', user_id).execute()
        if not user_result.data:
            flash('User not found', 'danger')
            return redirect(url_for('logout'))
        ...
        return render_template('group.video.call.html',
                             ...
                             current_user_name=current_user_data['full_name'],
...
# ✅ UPDATED: disconnect uses session
...
# ✅ SEND MESSAGE - use session user_id
...
def handle_send_message(data):
    ...
    sender_name = session.get('full_name', 'User')
...
def handle_ping_receiver(data):
    ...
    emit('call_ping', {
        'from_id': user_id,
        'from_name': session.get('full_name', 'User'),
        'timestamp': get_utc_time()
    }, room=target_id)
...
def handle_call_user(data):
    ...
    emit('incoming_call', {
        'caller_id': user_id,
        'caller_name': session.get('full_name', 'User'),
        'call_type': call_type,
        'offer': offer,
...
# ============================================
# ✅ FIXED: edit_message route - session-based
# ============================================
...
def edit_message_route():
    try:
        ...
        if edit_message(message_id, user_id, new_content):
...
# ============================================
# ✅ FIXED: delete_message_route - session-based
# ============================================
...
def delete_message_route(message_id):
    try:
        ...
        if msg['sender_id'] != user_id and delete_for == 'everyone':
            return jsonify({'error': 'Not authorized'}), 403
        ...
        if delete_for == 'everyone':
            ...
            socketio.emit('message_deleted', {'message_id': message_id, 'for_everyone': False}, room=str(user_id))
...
# ============================================
# ✅ FIXED: react_to_message route - session-based
# ============================================
...
def react_to_message_route():
    try:
        ...
        socketio.emit('reaction_updated', {'message_id': message_id, 'reactions': new_reactions}, room=str(user_id))
        
        return jsonify({'success': True, 'reactions': new_reactions})
    except Exception as e:
        logger.error(f"React to message error: {e}")
        return jsonify({'success': False, 'message': str(e)}), 500

# ============================================
# ✅ FIXED: upload route (private chat) - session-based
# ============================================
...
def upload_file():
    try:
        ...
        if saved_msg:
            ...
            socketio.emit('new_message', msg_dict, room=str(user['id']))
...
# ============================================
# ✅ FIXED: upload_audio route (private chat) - session-based
# ============================================
...
def upload_audio():
    try:
        ...
        file_path = f"private_chat_audio/{uuid.uuid4()}_voice_recording.webm"
        audio_bytes = audio.read()
