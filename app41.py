...
# ============================================
# ✅ FIXED ORDER HISTORY ROUTE - GUARANTEED WORKING
# ============================================

@app.route('/order_history')
@login_required
def order_history():
    """Display user's order history - FIXED VERSION"""
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    SELECT 
                        ...
                        o.delivery_location,
                        o.status,
                        TO_CHAR(o.order_date, 'DD Mon YYYY, HH12:MI AM') as formatted_date,
                ...
                print(f"✅ [ORDER_HISTORY] Found {len(orders)} orders for user {session['user_id']}")
                
                for order in orders:
                    items_list = []
                    order_id = order['order_id']
                    
                    # 🚨 Get items from order_items table (GUARANTEED WORKING)
                    cur.execute("""
                        SELECT 
                            oi.item_type,
                            oi.item_id,
                    ...
                    # If order_items is empty, try JSON fallback
                    if not items_list:
                        print(f"⚠️ [ORDER_HISTORY] No items in order_items for order #{order_id}, trying JSON...")
                        # Try to get items from JSON column (legacy support)
                        cur.execute("SELECT items FROM orders WHERE order_id = %s", (order_id,))
                        order_data = cur.fetchone()
                        
                        if order_data and order_data['items']:
                            try:
                                json_items = json.loads(order_data['items'])
                                ...
                                print(f"❌ [ORDER_HISTORY] JSON parse error for order #{order_id}: {json_error}")
                    
                    print(f"✅ [ORDER_HISTORY] Order #{order_id} will display {len(items_list)} items")
                    ...
                    orders_list.append({
                        ...
                        'items': items_list  # ✅ Now guaranteed to have items
                    })
            ...
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    SELECT 
                        ...
                        STRING_AGG(oi.item_type || '#' || oi.item_id, ', ') as items_list
                    FROM orders o
