            ...
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if user and check_password_hash(user['password'], password):
                        ...
                        return render_template('login.html')  # ✅ FIXED: Remove extra )
...
# ============================================
# ✅ UPDATED CART ROUTE WITH CLOUDINARY PHOTOS
# ============================================

@app.route('/cart')
@login_required
def cart():
    """Display user's cart WITH CLOUDINARY PHOTOS"""
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                # ✅ GET CLOUDINARY PHOTOS LIKE SERVICES/MENU ROUTES
                cart_items = []
                total_amount = 0
                
                # Get Cloudinary images for services
                service_photos = {}
                menu_photos = {}
                
                try:
                    # Fetch service photos from Cloudinary
                    cloudinary_services = cloudinary.api.resources(
                        type="upload",
                        prefix=SERVICES_FOLDER,
                        max_results=100
                    ...
                    for resource in cloudinary_services.get('resources', []):
                        filename = os.path.splitext(os.path.basename(resource['public_id']))[0]
                        service_name = filename.replace('_', ' ').title()
                        service_photos[service_name.lower()] = resource['secure_url']
                    
                    # Fetch menu photos from Cloudinary
                    cloudinary_menu = cloudinary.api.resources(
                        type="upload",
                        prefix=MENU_FOLDER,
                        max_results=100
                    ...
                    for resource in cloudinary_menu.get('resources', []):
                        filename = os.path.splitext(os.path.basename(resource['public_id']))[0]
                        menu_name = filename.replace('_', ' ').title()
                        menu_photos[menu_name.lower()] = resource['secure_url']
                        
                except Exception as cloudinary_error:
                    print(f"Cloudinary error in cart: {cloudinary_error}")
                
                # Process cart items with Cloudinary photos
                for item in cart_items_data:
                    item_name = item['item_name'].lower()
                    item_photo = item['db_photo']
                    
                    # ✅ USE CLOUDINARY FIRST (LIKE SERVICES/MENU ROUTES)
                    if item['item_type'] == 'service':
                        if item_name in service_photos:
                            item_photo = service_photos[item_name]
                        elif not item_photo:
                            # Fallback to default service photo
                            item_photo = "https://res.cloudinary.com/demo/image/upload/v1633427556/sample_service.jpg"
                    else:  # menu item
                        if item_name in menu_photos:
                            item_photo = menu_photos[item_name]
                        elif not item_photo:
                            # Fallback to default menu photo
                            item_photo = "https://res.cloudinary.com/demo/image/upload/v1633427556/sample_food.jpg"
