...
# ✅ FIXED: UUID type error in referral route – session user_id now properly converted to UUID
    ...
    try:
        ...
        if new_total >= 1000 and not user.get('reward_given', False):
            ...
            if referral_mobile:
                ...
                # ✅ FIXED: Use 'phone' column instead of 'mobile'
...
@login_required
def referral():
    """Referral & Wallet page - Complete referral system"""
    try:
        user_id = session['user_id']
        user_phone = session.get('phone')
        
        # ✅ FIX: If user_id is not a valid UUID (e.g., integer), fetch actual UUID from database
        uuid_pattern = re.compile(r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$', re.I)
        if not uuid_pattern.match(str(user_id)):
            print(f"⚠️ Session user_id '{user_id}' is not a UUID. Fetching correct UUID by phone...")
            if user_phone:
                users_by_phone = supabase_execute('users', 'select', conditions={'phone': user_phone}, use_admin=True)
                if users_by_phone:
                    correct_uuid = users_by_phone[0]['id']
                    session['user_id'] = correct_uuid
                    user_id = correct_uuid
                    print(f"✅ Updated session user_id to UUID: {correct_uuid}")
                else:
        ...
        try:
            ...
            reward_transactions = supabase_execute('transactions', 'select',
                                                   conditions={'user_id': user_id_str, 'type': 'referral_reward'},
